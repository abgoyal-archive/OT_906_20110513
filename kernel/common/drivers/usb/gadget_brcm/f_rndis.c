...
invalid:
		...
		pr_info("USB gadget:invalid control req%02x.%02x v%04x i%04x l%d\n",
			ctrl->bRequestType, ctrl->bRequest,
			w_value, w_index, w_length);
	}
	...
	if (value >= 0) {
		...
		if (value < 0)
			pr_info("USB Gadget:rndis response on err %d\n", value);//ERROR(cdev, "rndis response on err %d\n", value);
	}
...
static int rndis_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	if (intf == rndis->ctrl_id) {
		...
		goto fail;
	pr_info("rndis_set_alt -- OK\n");
	return 0;
fail:
	pr_info("rndis_set_alt --Failed\n");
	return -EINVAL;
}
...
static void rndis_disable(struct usb_function *f)
{
	...
	if (!rndis->notify->driver_data) {
		rndis->port.in_ep->driver_data = NULL; //for switching between different usb config
		return;
	}
	
	pr_info("rndis_disable\n");
...
/**
 * rndis_interface_enable() - to enable/disable the adb interface 
...
{	
	...
	if (rndis) {		
		pr_info("rndis_interface_enable(%s)\n",enable ? "true" : "false");
		set_usb_interface(&rndis->port.func, rndis_control_intf.bInterfaceNumber, enable);
		set_usb_interface(&rndis->port.func, rndis_data_intf.bInterfaceNumber, enable);
		
		/* If it is enabled, it needs to be disabled before software disconnects the USB devices (Android_force_reenum).... */
		if (enable)	
			rndis_disable(&rndis->port.func);			
	} else
