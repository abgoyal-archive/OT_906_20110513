...
*
* 	@file	drivers/input/misc/smb380.c
*
...
int smb380_soft_reset()
{
	...
	comres =
	    p_smb380->SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr,
					    SMB380_SOFT_RESET__REG, &data, 1);
	return comres;
}
...
int smb380_update_image()
{
	...
	comres =
	    p_smb380->SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr,
					    SMB380_UPDATE_IMAGE__REG, &data, 1);
	return comres;
}
...
int smb380_set_image(smb380regs_t *smb380Image)
{
	...
	if (p_smb380 == 0)
		return E_SMB_NULL_PTR;
	comres =
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_EE_W__REG,
					   &data, 1);
	data = SMB380_SET_BITSLICE(data, SMB380_EE_W, SMB380_EE_W_ON);
	comres =
	    p_smb380->SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr,
					    SMB380_EE_W__REG, &data, 1);
	comres =
	    p_smb380->SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr,
					    SMB380_IMAGE_BASE,
					    (unsigned char *)smb380Image,
					    SMB380_IMAGE_LEN);
	comres =
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_EE_W__REG,
					   &data, 1);
...
int smb380_get_image(smb380regs_t *smb380Image)
{
	...
	comres =
	    p_smb380->SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr,
					    SMB380_EE_W__REG, &data, 1);
	comres =
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr,
					   SMB380_IMAGE_BASE,
					   (unsigned char *)smb380Image,
					   SMB380_IMAGE_LEN);
	data = SMB380_SET_BITSLICE(data, SMB380_EE_W, SMB380_EE_W_OFF);
...
int smb380_get_offset(unsigned char xyz, unsigned short *offset)
{
	...
	comres =
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr,
					   (SMB380_OFFSET_X_LSB__REG + xyz),
					   &data, 1);
	data = SMB380_GET_BITSLICE(data, SMB380_OFFSET_X_LSB);
	*offset = data;
	comres +=
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr,
					   (SMB380_OFFSET_X_MSB__REG + xyz),
					   &data, 1);
	*offset |= (data << 2);
	return comres;
}
...
int smb380_set_wake_up_pause(unsigned char wup)
{
	...
	comres =
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr,
					   SMB380_WAKE_UP_PAUSE__REG, &data, 1);
	data = SMB380_SET_BITSLICE(data, SMB380_WAKE_UP_PAUSE, wup);
	comres +=
	    p_smb380->SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr,
					    SMB380_WAKE_UP_PAUSE__REG, &data,
					    1);
	return comres;
}
...
int smb380_set_any_motion_threshold(unsigned char th)
{
	...
	comres =
	    p_smb380->SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr,
					    SMB380_ANY_MOTION_THRES__REG, &th,
					    1);
...
int smb380_get_any_motion_threshold(unsigned char *th)
{
	...
	comres =
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr,
					   SMB380_ANY_MOTION_THRES__REG, th, 1);
...
int smb380_set_any_motion_count(unsigned char amc)
{
	...
	comres =
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr,
					   SMB380_ANY_MOTION_DUR__REG, &data,
					   1);
...
int smb380_set_interrupt_mask(unsigned char mask)
{
	...
	comres =
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_CONF1_REG,
					   &data[1], 1);
	comres +=
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_CONF2_REG,
					   &data[3], 1);
	...
	comres +=
	    p_smb380->SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr,
					    SMB380_CONF1_REG, &data[1], 1);
	comres +=
	    p_smb380->SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr,
					    SMB380_CONF2_REG, &data[3], 1);
...
int smb380_read_accel_x(short *a_x)
{
	...
	comres =
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr,
					   SMB380_ACC_X_LSB__REG, data, 2);
	*a_x =
	    SMB380_GET_BITSLICE(data[0],
				SMB380_ACC_X_LSB) | SMB380_GET_BITSLICE(data[1],
									SMB380_ACC_X_MSB)
	    << SMB380_ACC_X_LSB__LEN;
	*a_x =
	    *a_x << (sizeof(short) * 8 -
		     (SMB380_ACC_X_LSB__LEN + SMB380_ACC_X_MSB__LEN));
	*a_x =
	    *a_x >> (sizeof(short) * 8 -
		     (SMB380_ACC_X_LSB__LEN + SMB380_ACC_X_MSB__LEN));
	return comres;
...
int smb380_read_accel_y(short *a_y)
{
	...
	comres =
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr,
					   SMB380_ACC_Y_LSB__REG, data, 2);
	*a_y =
	    SMB380_GET_BITSLICE(data[0],
				SMB380_ACC_Y_LSB) | SMB380_GET_BITSLICE(data[1],
									SMB380_ACC_Y_MSB)
	    << SMB380_ACC_Y_LSB__LEN;
	*a_y =
	    *a_y << (sizeof(short) * 8 -
		     (SMB380_ACC_Y_LSB__LEN + SMB380_ACC_Y_MSB__LEN));
	*a_y =
	    *a_y >> (sizeof(short) * 8 -
		     (SMB380_ACC_Y_LSB__LEN + SMB380_ACC_Y_MSB__LEN));
	return comres;
...
int smb380_read_accel_z(short *a_z)
{
	...
	comres =
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr,
					   SMB380_ACC_Z_LSB__REG, data, 2);
	*a_z =
	    SMB380_GET_BITSLICE(data[0],
				SMB380_ACC_Z_LSB) | SMB380_GET_BITSLICE(data[1],
									SMB380_ACC_Z_MSB)
	    << SMB380_ACC_Z_LSB__LEN;
	*a_z =
	    *a_z << (sizeof(short) * 8 -
		     (SMB380_ACC_Z_LSB__LEN + SMB380_ACC_Z_MSB__LEN));
	*a_z =
	    *a_z >> (sizeof(short) * 8 -
		     (SMB380_ACC_Z_LSB__LEN + SMB380_ACC_Z_MSB__LEN));
	return comres;
 ...
 * \note: an output of 0 equals -30 degree C, 1 LSB equals 0.5 degree C
 */
int smb380_read_temperature(unsigned char *temp)
{
	...
	comres =
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr,
					   SMB380_TEMPERATURE__REG, temp, 1);
	return comres;
...
int smb380_read_accel_xyz(smb380acc_t *acc)
{
	...
	if (p_smb380 == 0)
		return E_SMB_NULL_PTR;
/*      printk("will read smb380 address 0x%x\n",p_smb380->dev_addr); */
	comres =
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr,
					   SMB380_ACC_X_LSB__REG, &data[0], 6);
/*      printk("read xyz data is %d,%d,%d,%d,%d,%d\n",data[0],data[1],data[2],data[3],data[4],data[5]); */
	acc->x = SMB380_GET_BITSLICE(data[0], SMB380_ACC_X_LSB) |
	    (SMB380_GET_BITSLICE(data[1], SMB380_ACC_X_MSB) <<
	     SMB380_ACC_X_LSB__LEN);
	acc->x = acc->x << (sizeof(short) * 8 -
		       (SMB380_ACC_X_LSB__LEN + SMB380_ACC_X_MSB__LEN));
	acc->x = acc->x >> (sizeof(short) * 8 -
		       (SMB380_ACC_X_LSB__LEN + SMB380_ACC_X_MSB__LEN));

	acc->y = SMB380_GET_BITSLICE(data[2], SMB380_ACC_Y_LSB) |
	    (SMB380_GET_BITSLICE(data[3], SMB380_ACC_Y_MSB) <<
	     SMB380_ACC_Y_LSB__LEN);
	...
	acc->z |= (SMB380_GET_BITSLICE(data[5], SMB380_ACC_Z_MSB) <<
	     SMB380_ACC_Z_LSB__LEN);
...
int smb380_set_any_motion_int(unsigned char onoff)
{
	...
	comres =
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr,
					   SMB380_EN_ANY_MOTION__REG, &data, 1);
	data = SMB380_SET_BITSLICE(data, SMB380_EN_ANY_MOTION, onoff);
	comres +=
	    p_smb380->SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr,
					    SMB380_EN_ANY_MOTION__REG, &data,
					    1);
...
int smb380_set_advanced_int(unsigned char onoff)
{
	...
	comres =
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr,
					   SMB380_ENABLE_ADV_INT__REG, &data,
					   1);
...
int smb380_set_new_data_int(unsigned char onoff)
{
	...
	comres =
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr,
					   SMB380_NEW_DATA_INT__REG, &data, 1);
...
int smb380_read_reg(unsigned char addr, unsigned char *data, unsigned char len)
{
	...
	comres =
	    p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr, addr, data, len);
	return comres;
...
int smb380_write_reg(unsigned char addr, unsigned char *data, unsigned char len)
{
	...
	comres =
	    p_smb380->SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, addr, data,
					    len);
