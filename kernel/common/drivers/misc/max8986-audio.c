...
int max8986_audio_set_mixer_input(int audio_path, max8986_mixer_input input)
{
	...
	{
		ret = max8986->read_dev(max8986, MAX8986_AUDIO_REG_HP_MIXER, &regVal);
		if(audio_path == MAX8986_AUDIO_HS_LEFT)
		{
			regVal &= ~AUDIO_MIXER_HPL_MIX_MASK;
			regVal |= (input << AUDIO_MIXER_HPL_MIX_BIT) & AUDIO_MIXER_HPL_MIX_MASK;
		}
		else
		{
			regVal &= ~AUDIO_MIXER_HPR_MIX_MASK;
			regVal |= (input << AUDIO_MIXER_HPR_MIX_BIT) & AUDIO_MIXER_HPR_MIX_MASK;
		}
		ret |= max8986->write_dev(max8986, MAX8986_AUDIO_REG_HP_MIXER, regVal);
	}
	else if(audio_path == MAX8986_AUDIO_SPK)
	{
		regVal = input & AUDIO_MIXER_SPK_MIX_BIT;
		ret = max8986->write_dev(max8986, MAX8986_AUDIO_REG_SP_MIXER, regVal);
	}
	return ret;
}
...
int max8986_set_input_mode(int input_path, int mode)
{
	...
	if(mode == MAX8986_INPUT_MODE_DIFFERENTIAL)
		...
		return -EINVAL;
	return max8986->write_dev(max8986, MAX8986_AUDIO_REG_INPUT_GAIN, regVal);
}
...
static int __devinit max8986_audio_probe(struct platform_device *pdev)
{
	...
	if(max8986->pdata && max8986->pdata->audio_pdata)
	{
		...
		max8986_audio_set_mixer_input(MAX8986_AUDIO_HS_RIGHT,audio_pdata->rhs_def_mixer_in);
		max8986_audio_set_mixer_input(MAX8986_AUDIO_HS_LEFT,audio_pdata->lhs_def_mixer_in);
		max8986_audio_set_mixer_input(MAX8986_AUDIO_SPK,audio_pdata->ihf_def_mixer_in);

		max8986_set_input_preamp_gain(MAX8986_INPUTA,audio_pdata->ina_def_preampgain);
		max8986_set_input_preamp_gain(MAX8986_INPUTB,audio_pdata->inb_def_preampgain);

		max8986_set_input_mode(MAX8986_INPUTA,audio_pdata->ina_def_mode);
		max8986_set_input_mode(MAX8986_INPUTB,audio_pdata->inb_def_mode);
	}
