 ...
 *
 * $Id: dhd_linux.c,v 1.65.4.9.2.12.2.104.4.22 2010/10/19 19:50:37 Exp $
 */
...
static struct wake_lock Wlan_Lock_Int_Clean; //Shri
static int Wake_Lock_Flag=0;
...
static int dhd_keep_alive_onoff(dhd_pub_t *dhd, int ka_on)
{
	...
	int str_len, buf_len;
	int res = 0;
	int keep_alive_period = (KEEP_ALIVE_PERIOD/12); /* in ms */
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	spin_lock_init(&dhd->sdlock); //move for initialize 
	spin_lock_init(&dhd->txqlock);

	wake_lock_init(&Wlan_Lock_Int_Clean, WAKE_LOCK_SUSPEND, "Wlan_Driver");//Shri
//	wake_lock(&Wlan_Lock_Int_Clean); //Shri
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	dhd->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 12;
...
dhd_detach(dhd_pub_t *dhdp)
{
	...
	if (dhdp) {
        ...
        {
            if(dhd->dpc_pid == -1) //if dhd->dpc_pid == -2 dhd->taskle not be initialed
...
dhd_module_cleanup(void)
{
	...
	if(Wake_Lock_Flag==1)
	{
		DHD_ERROR(("%s: WAKE_UNLOCK CALLED %d\n", __FUNCTION__,Wake_Lock_Flag)); //Shri
		wake_unlock(&Wlan_Lock_Int_Clean); //Shri
		Wake_Lock_Flag=0;
	}
	wake_lock_destroy(&Wlan_Lock_Int_Clean);
}
...
dhd_os_open_image(char *filename)
{
	...
	DHD_ERROR(("Firmware File Name %s\n", filename)); //Shri
	if ( (strcmp(filename, "/system/etc/wifi/RC_248_WPA.bin")==0))
	{
		DHD_ERROR(("This is AP Mode Firmware Setting WAKE_LOCK %s\n", filename));
		wake_lock(&Wlan_Lock_Int_Clean); //Shri
		Wake_Lock_Flag=1;
