...
void bcmsdhc_sdio_host_force_scan( struct platform_device *pdev);
...
static void bcmsdhc_init(struct bcmsdhc_host *host)
{
	u32 intmask;
...
static void bcmsdhc_finish_data(struct bcmsdhc_host *host)
{
	...
	if (host->flags & SD_AUTO_ISSUE_CMD12) {
...
static void bcmsdhc_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (mmc_resp_type(mrq->cmd) == MMC_RSP_NONE) {
		...
		wake_lock_timeout(&host->sdhc_wakelock, msecs_to_jiffies(5000));
	}
#endif
...
int bcmsdhc_get_cd(struct mmc_host *mmc_host)
{
	...
	if ((host->irq_cd > 0) &&
		(gpio_get_value(host->irq_cd) != host->bcm_plat->card_in_state)) {
...
static void bcmsdhc_tasklet_card(unsigned long param)
{
	...
	if (state != host->bcm_plat->card_in_state) {
		...
		if (host->cmd == NULL) {
			...
			if (host->card_present == true) {
				if (host->cmd->retries >= 1) {
					...
					pr_info("%s: Timeout waiting for command complete interrupt after 5 retries.\n",
						mmc_hostname(host->mmc));
				}
			}
		}
...
static irqreturn_t bcmsdhc_irq(int irq, void *dev_id)
{
	...
	if (wIntStatus != 0) {
		...
		if (wIntStatus) {
			pr_info
			    (" SDHC interrupt state do not clear .State=0x%08x.\n",
			     wIntStatus);
			bcmsdhc_dumpregs(host);
			writel(wIntStatus,
			       host->ioaddr + SDHC_NORMAL_INT_STATUS);
...
static int __init bcmsdhc_probe(struct platform_device *pdev)
{
	...
	if (pdev->resource == NULL || pdev->num_resources < 2) {
		pr_err(
		       "%s:SDHC: Invalid resource and num_resources\n",
		       __func__);
		return -ENXIO;
	}
	...
	pr_info("sdhc[%d], start %x - end %x, irq %d cd %d %p\n",
	       host->sdhc_slot, memres->start, memres->end,
	...
	pr_info(
	       "SDHC:slot[%d]=>Host version = %d ,Host capabilities =0x%x\n",
...
*/
	...
	if (host->irq_cd > 0) {
		...
		if (gpio_get_value(host->irq_cd) != bcm_plat->card_in_state) {
			host->card_present = false;
		}
	}
...
static int bcmsdhc_resume(struct platform_device *pdev)
{
	...
	if (host) {
		...
		if(bcmsdhc_get_cd(host->mmc)==1 && (host->mmc->card &&
