...
/*Power Voltage Setting*/
	...
	/*Power on Setting*/
		...
		{WRITE_INDEX,   LCD_OUT(0x0018)},   {WRITE_DATA,   LCD_OUT(0x0033)},/*I/P_RADJ,N/P_RADJ, Normal mode 80Hz*/
...
void
lcd_setup_for_data(LCD_dev_info_t *dev)
{
		/*lcd_enable_ce(false);*/
...
static int
lcd_send_cmd_sequence(Init_Seq_t *init)
{
		int i;
		...
		LCD_DEBUG("Index:0x%04x(0x%04x), Data:0x%04x(0x%04x)\n", init[i].data>>1, init[i].data, init[i+1].data>>1, init[i+1].data);
