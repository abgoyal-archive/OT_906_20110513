...
static int gLcdfbEarlySuspendStopDraw;
#endif
...
#ifdef CONFIG_ENABLE_FPS
static int gMemRefreshFreq = HZ / 2;
#endif
...
static struct ctl_table gSysCtlFb[] = {
	...
	{
	 .ctl_name = 2,
	 .procname = "refresh-freq-ui",
	 .data = &gMemRefreshFreq,
	 .maxlen = sizeof(int),
	 .mode = 0644,
	 .proc_handler = &proc_dointvec},
#endif
...
/****************************************************************************
*
*  lcdfb_refresh_statistics
*
*   calculate the ui refresh frequence, return T in ms.
*
***************************************************************************/

static void lcdfb_refresh_statistics()
{
#if 0
	static u64 time = 0;
	static u32 frames = 0;
	u64 interval, frequnce;

	if (frames >= 10) {
		interval = sched_clock() - time;
		frequnce = 10000000000LL;/* 1second * 10frames */
		if (interval != 0)
			do_div(frequnce, interval);

		time = sched_clock();
		frames = 1;
		gMemRefreshFreq = (int)(frequnce);
	} else if (frames > 0) {
		frames++;
	} else {
		time = sched_clock();
	...
	if (frames > 0) {
		frames++;
		do_gettimeofday(&ctv);
		interval = (ctv.tv_sec - ltv.tv_sec) * 1000 + (ctv.tv_usec - ltv.tv_usec) / 1000;
		if (interval >= 1000) {
			gMemRefreshFreq = frames * 1000 / interval;
			do_gettimeofday(&ltv);
			frames = 1;
		}
	} else {
	...
	if (time > 0) {
		frames++;
		interval = sched_clock() - time;
		if (interval >= 1000000000LL) { /* larger than 1 seconds */
			do_div(interval, 1000000LL);
			gMemRefreshFreq = frames * 1000 / (u32)interval;
			time = sched_clock();
			frames = 0;
		}
	} else {
				 ...
				 LCD_FillRectColor_t *rect_c)
{
	...
	if (rect_c->width == 0 || rect_c->height == 0 ||
		...
		pr_err(
		       "error lcdfb_fillrect_color: Invalid parameters\n"
...
#else
	pr_info(
	       "Proc ADDRs gSysCtl 0x%x gsysctlfb0 0x%x gsysctlfb1 0x%x\n",
