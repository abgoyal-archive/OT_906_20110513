...
// Semaphore Create
    ...
    {    
        sSemCslCamTask = (UInt32)OSDAL_SEMA_CREATE (0, OSSUSPEND_PRIORITY);
        if (sSemCslCamTask ==  NULL)
        {
            printk(KERN_INFO  "[cslCamTaskInit][Error] : sSemCslCamTask: FAILED \n" );
            success |= -1;
        }
        else
        {
            OSDAL_SEMA_NAMECHANGE ( sSemCslCamTask, "SEMCSLCAMTASK");
        }
    }
...
// Task Create
    if (sCslCamTask == NULL)
    { 
        sCslCamTask = (UInt32)OSDAL_TASK_CREATE (cslCamFrameTask, TASKNAME_CSLCAM,0,0);
        if( sCslCamTask == NULL )
        {
            printk(KERN_INFO  "[cslCamTaskInit][Error]: cslCamFrameTask: FAILED \n" );
            success |= -1;
        }
    }
...
// Hisr Create
    ...
    if (sHisrCslCam == NULL)
    {   // V 
        sHisrCslCam = (UInt32)OSDAL_IRQ_CREATE( &cslCamFrameHisr, HISRNAME_CSLCAM, 0, 0);
        if( sHisrCslCam == NULL )
        {
            printk(KERN_INFO  "[cslCamTaskInit][Error]: cslCamFrameHisr: FAILED \n") );
            success |= -1;
        }
    }
#endif
...
// Hisr Destroy
    if (sHisrCslCam != NULL)
    {    
        printk(KERN_INFO  "[cslCamTaskDeInit][Info] : Destroy sCslCamTask \n" );
        // V OSDAL_IRQ_DELETE (sHisrCslCam);
        sHisrCslCam = NULL;
    }
// Task Destroy
    if (sCslCamTask != NULL)
    {
        ...
        if (sem_count != 0)
        {
            printk(KERN_INFO  "[cslCamTaskDeInit][Info]: sSemCslCamTask count=%d: \n", sem_count );
        // Wait for CSL Camera task to finish
            while ( OSDAL_SEMA_COUNT(sSemCslCamTask) != 0 );
        }
        printk(KERN_INFO  "[cslCamTaskDeInit][Info] : Destroy sCslCamTask \n" );
        OSDAL_TASK_DELETE (sCslCamTask);
        sCslCamTask = NULL;
    }
// Semaphore Destroy
   if (sSemCslCamTask != NULL)
    {    
        printk(KERN_INFO  "[cslCamTaskDeInit][Info] : Destroy sSemCslCamTask \n" );
        OSDAL_SEMA_DELETE (sSemCslCamTask);
        sSemCslCamTask = NULL;
    }  
...
//	printk(KERN_INFO"AT start %d INT 0x%x\n",camH->state.bufferIndex,camH->state.rx_status);
