...
static void cslLcdcUpdateTask (void)
{
    ...
    for (;;)
    {
        ...
        if ( osStat != OSSTATUS_SUCCESS)
        {
            ...
            {
                LCD_DBG ( LCD_DBG_ID, "[CSL LCDC] cslLcdcUpdateTask: "
                    "ERR EOFDMA Timeout!\n\r" );
                res = CSL_LCD_OS_TOUT;
            }
            else
        ...
        if( !updMsg.lcdH->hasLock ) OSSEMAPHORE_Release ( lcdcSema );
...
Boolean static cslLcdcOsInit (void)
{
    ...
    updReqQ = OSQUEUE_Create( FLUSH_Q_SIZE,
        sizeof(LCDC_UPD_REQ_MSG_T), OSSUSPEND_PRIORITY);

    if (!updReqQ)
    {
        LCD_DBG ( LCD_DBG_ID, "[CSL LCDC] cslLcdcOsInit: ERR Queue creation\n");
        res = FALSE;
    }
    else
    {
    ...
    if (!task)
    {
        LCD_DBG ( LCD_DBG_ID, "[CSL LCDC] cslLcdcOsInit: ERR Task creation!\n");
        res = FALSE;
    }
    ...
    if (!eofDmaSema)
    {
        LCD_DBG ( LCD_DBG_ID, "[CSL LCDC] cslLcdcOsInit: ERR Sema Creation!\n");
        res = FALSE;
    }
    else
    {
...
//
//*****************************************************************************
Boolean static cslLcdcSetIoWidth(
    UInt32 bus_width
    )
{
    UInt32 val, mask;
    ...
    *(volatile UInt32*) (io_p2v(SYSCFG_BASE_ADDR + SYSCFG_IOCR0_OFFSET)) &= ~mask;;
    *(volatile UInt32*) (io_p2v(SYSCFG_BASE_ADDR + SYSCFG_IOCR0_OFFSET)) |= val;
...
void static cslLcdcSetIoEnable ( Boolean enable )
{
    ...
    if ( enable )
        *(volatile cUInt32*) (io_p2v(SYSCFG_IOCR3)) &= ~SYSCFG_IOCR3_LCD_DIS;
    else
        *(volatile cUInt32*) (io_p2v(SYSCFG_IOCR3)) |= SYSCFG_IOCR3_LCD_DIS;
#else
    if ( enable )
        BRCM_WRITE_REG_FIELD ( io_p2v(SYSCFG_BASE_ADDR), SYSCFG_IOCR3, LCD_DIS, 0 );
    else
        BRCM_WRITE_REG_FIELD ( io_p2v(SYSCFG_BASE_ADDR), SYSCFG_IOCR3, LCD_DIS, 1 );
#endif
}
...
void static cslLcdcSetIoDrive ( CSL_LCDC_IO_DRIVE_t io )
{
    // ATHENA has slewFast ?
    if ( io.slewFast )
        *(volatile cUInt32*) (io_p2v(SYSCFG_IOCR4)) &= ~SYSCFG_IOCR4_LCD_slew;
    else
        *(volatile cUInt32*) (io_p2v(SYSCFG_IOCR4)) |= SYSCFG_IOCR4_LCD_slew;

    *(volatile cUInt32*) (io_p2v(SYSCFG_IOCR4)) &= ~SYSCFG_IOCR4_LCD_drive;
    *(volatile cUInt32*) (io_p2v(SYSCFG_IOCR4)) |= cslLcdcGetBitFieldVal (
        SYSCFG_IOCR4_LCD_drive, io.driveStrength);
}
...
void static cslLcdcParSetIntfType ( CHAL_PAR_BUS_TYPE_t busType )
{
    ...
    if ( busType == CHAL_BUS_M68 )
        BRCM_WRITE_REG_FIELD ( io_p2v(SYSCFG_BASE_ADDR), SYSCFG_IOCR0, M68, 1 );
    else
        BRCM_WRITE_REG_FIELD ( io_p2v(SYSCFG_BASE_ADDR), SYSCFG_IOCR0, M68, 0 );
#else
#if defined (_BCM21551_)
    if ( busType == CHAL_BUS_M68 )
...
#if defined (_ATHENA_)
    if ( busType == CHAL_BUS_M68 )
        *(volatile cUInt32*) (io_p2v(SYSCFG_BASE_ADDR) + SYSCFG_IOCR0_OFFSET) |=
            SYSCFG_IOCR0_M68;
    else
        *(volatile cUInt32*) (io_p2v(SYSCFG_BASE_ADDR) + SYSCFG_IOCR0_OFFSET) &=
            ~SYSCFG_IOCR0_M68;
...
INLINE static void cslLcdcWaitWhileFifoIsFull (void)
{
    ...
    while (chal_lcdc_rd_status (cslLcdcDrv.chalH) & LCDC_STATUS_FFFULL_MASK);
}
...
INLINE static void cslLcdcWaitWhileLcdcIsBusy (void)
{
    ...
    while (chal_lcdc_rd_status (cslLcdcDrv.chalH) & LCDC_STATUS_LCD_BUSY_MASK);
}
#endif
...
INLINE static void cslLcdcWaitUntilFifoIsEmpty (void)
{
    ...
    while (!(chal_lcdc_rd_status (cslLcdcDrv.chalH) & LCDC_STATUS_FFEMPTY_MASK));
}
...
INLINE static void cslLcdcWaitUntilFifoHasData (void)
{
    ...
    while (!(chal_lcdc_rd_status (cslLcdcDrv.chalH) & LCDC_STATUS_RREADY_MASK));
...
static CSL_LCD_RES_T cslLcdcDmaStart ( LCDC_UPD_REQ_MSG_T* updMsg )
{
    ...
    updMsg->dmaChInfo.srcBstSize     = DMA_BURST_SIZE_4;
    updMsg->dmaChInfo.dstBstSize     = DMA_BURST_SIZE_4;
...
//  updMsg->dmaChInfo.priority       = 0;                 // ?
	...
	updMsg->dmaData.pBufList   = (Dma_Buffer_List *)updMsg->updReq.buff;

        if ( DMADRV_Bind_Data_Ex (updMsg->dmaCh, &updMsg->dmaData, &dmaLLI)
            != DMADRV_STATUS_OK )
        {
            LCD_DBG ( LCD_DBG_ID, "[CSL LCDC] cslLcdcDmaStart:"
        ...
        if ( DMADRV_Start_Transfer_Ex(updMsg->dmaCh, dmaLLI) != DMADRV_STATUS_OK )
        {
            LCD_DBG ( LCD_DBG_ID, "[CSL LCDC] cslLcdcDmaStart:"
...
//
//*****************************************************************************
CSL_LCD_RES_T  cslLcdcDbi2cHal (
    CSL_LCDC_DBI_CTRL_T* busCfg,
    LCDC_HANDLE          lcdc
    )
{
    ...
    switch ( busCfg->cfg.colModeIn )
    {
        ...
        case LCD_IF_CM_I_RGB888P:
            ...
            break;
        default:
            LCD_DBG ( LCD_DBG_ID, "[CSL LCDC DBI]: Invalid Input Color Mode!\n");
    ...
    switch ( busCfg->cfg.colModeOut )
    {
        ...
        case LCD_IF_CM_O_RGB332:
            ...
            break;
        default:
            LCD_DBG ( LCD_DBG_ID, "[CSL LCDC DBI]: Invalid Output Color Mode!\n");
            return ( CSL_LCD_COL_MODE );
    }
...
static void cslLcdcDbiSelect ( LCDC_HANDLE lcdc )
{
    ...
    if ( lcdc->config.ctrlDbi.cfg.busType == CHAL_BUS_DBI_B )
        chal_lcdc_dbib_set_speed (cslLcdcDrv.chalH, &lcdc->config.ctrlDbi.speed);
    else
        chal_lcdc_dbic_set_speed (cslLcdcDrv.chalH, &lcdc->config.ctrlDbi.speed);
...
//
//*****************************************************************************
CSL_LCD_RES_T  cslLcdcPar2cHal (
    CSL_LCDC_PAR_CTRL_T* busCfg,
    LCDC_HANDLE          lcdc
    )
{
    ...
    switch ( busCfg->cfg.colModeIn )
    {
        ...
        case LCD_IF_CM_I_RGB565P:
            ...
            break;
        default:
            LCD_DBG ( LCD_DBG_ID, "[CSL LCDC PAR]: Invalid Input Color Mode!\n");
            return ( CSL_LCD_COL_MODE );
    }
    ...
    switch ( busCfg->cfg.colModeOut )
    {
        ...
        case LCD_IF_CM_O_RGB332:
            break;
        default:
            LCD_DBG ( LCD_DBG_ID, "[CSL LCDC PAR]: Invalid Output Color Mode!\n");
            return ( CSL_LCD_COL_MODE );
    }
...
CSL_LCD_RES_T CSL_LCDC_WrCmnd (CSL_LCD_HANDLE lcdcH, UInt32 command)
{
    ...
    if( !lcdc->hasLock ) OSSEMAPHORE_Obtain ( lcdcSema, TICKS_FOREVER );
    ...
    if( !lcdc->hasLock ) OSSEMAPHORE_Release ( lcdcSema );
...
//
//*****************************************************************************
CSL_LCD_RES_T CSL_LCDC_PAR_SetSpeed (
...
//
//*****************************************************************************
CSL_LCD_RES_T CSL_LCDC_DBI_SetSpeed (
    CSL_LCD_HANDLE      lcdcH,
...
//
//*****************************************************************************
CSL_LCD_RES_T CSL_LCDC_DBI_RdAccess (
    CSL_LCD_HANDLE          lcdcH,
    ...
    )
{
    ...
    if ( lcdc->usesCE ) chal_lcdc_par_set_ce (cslLcdcDrv.chalH, TRUE);
#if (CHIP_REVISION >= 20)
    if ( lcdc->uses888U ) chal_lcdc_par_set_rgb888u (cslLcdcDrv.chalH, TRUE );
#endif
...
#endif
        if ( lcdc->usesCE ) chal_lcdc_par_set_ce (cslLcdcDrv.chalH, FALSE);

#if (CHIP_REVISION >= 20)
        if ( lcdc->uses888U ) chal_lcdc_par_set_rgb888u (cslLcdcDrv.chalH, FALSE );
#endif
    ...
    {   // NON_BLOCKING
        status = OSQUEUE_Post ( updReqQ, (QMsg_t*) &updM, TICKS_NO_WAIT );
        if ( status != OSSTATUS_SUCCESS)
        {
            LCD_DBG ( LCD_DBG_ID, "[CSL LCDC] CSL_LCDC_Update: "
...
//
//*****************************************************************************
CSL_LCD_RES_T CSL_LCDC_DBI_Open (
    pCSL_LCDC_DBI_CTRL      busCfg,
    CSL_LCD_HANDLE*         lcdcH
    )
{
    LCDC_HANDLE             lcdc;
...
//
//*****************************************************************************
CSL_LCD_RES_T  CSL_LCDC_PAR_Open (
    pCSL_LCDC_PAR_CTRL   busCfg,
    CSL_LCD_HANDLE*      lcdcH
    )
{
...
CSL_LCD_RES_T CSL_LCDC_Init ( pCSL_LCDC_INIT ctrlInit )
{
    ...
    if ( !cslLcdcDrv.init )
    {
	    ...
	    if(ctrlInit->lcdc_base_address)
                cslLcdcDrv.chalH = chal_lcdc_init (ctrlInit->lcdc_base_address);
	    else
                cslLcdcDrv.chalH = chal_lcdc_init ( LCDC_BASE_ADDR );
...
//
//*****************************************************************************
CSL_LCD_RES_T CSL_LCDC_Enable_CE (
	CSL_LCD_HANDLE	lcdcH,
	bool		enable
	)
{
