...
"	Turn on the digital slope gain
	**/
	...
	if( mic == AUDDRV_DUAL_MIC_DIGI12
		...
		|| mic == AUDDRV_DUAL_MIC_ANALOG_DIGI2 
	  )
	{
	...
	switch(input_path) 
	{
		case AUDDRV_VOICE_INPUT:
			...
			if( local_mic_selection_voice == AUDDRV_DUAL_MIC_DIGI12
				...
				|| local_mic_selection_voice == AUDDRV_DUAL_MIC_ANALOG_DIGI2 
			  )
			{
				Log_DebugPrintf(LOGID_AUDIO, "\n\r\t* AUDDRV_EnableHWInput calls chal_audiovipath2_Enable *\n\r" );
				//set adc2_sw_en in DSP_AUDIO_ADCCONTROL[5] for enabling 2nd digital mic
				...
				)
{
	...
	switch(mic)
	{
	    ...
	    case AUDDRV_DUAL_MIC_ANALOG_DIGI2:
			//rule #1
			chal_audiomic_PowerOnADC( mic_handle, TRUE );
...
{
	if (   voice_mic_current != AUDDRV_MIC_MAIN               && audio_mic_current != AUDDRV_MIC_MAIN
		&& voice_mic_current != AUDDRV_MIC_AUX                && audio_mic_current != AUDDRV_MIC_AUX
		&& voice_mic_current != AUDDRV_DUAL_MIC_ANALOG_DIGI1  && audio_mic_current != AUDDRV_DUAL_MIC_ANALOG_DIGI1
		&& voice_mic_current != AUDDRV_DUAL_MIC_ANALOG_DIGI2  && audio_mic_current != AUDDRV_DUAL_MIC_ANALOG_DIGI2
		)
		return TRUE;
	else
		return FALSE;
...
static void auddrv_pwrOffUnusedMic( void )
{
	...
	//main mic
	if ( auddrv_mic_not_used( AUDDRV_MIC_MAIN ) && micPoweredOn[AUDDRV_MIC_MAIN]== TRUE )
	{
		if( auddrv_analog_mic_not_used() )
		{
			...
			micPoweredOn[ AUDDRV_MIC_MAIN ]= FALSE;
		}
	}

	//aux mic  (aux mic can not be part of dual-mic with digi mic)
	if ( auddrv_mic_not_used( AUDDRV_MIC_AUX ) && micPoweredOn[AUDDRV_MIC_AUX]== TRUE )
	{
		if( auddrv_analog_mic_not_used() )
		{
			//rule #4
			...
			chal_audiomic_SetAuxBiasLevelHigh( mic_handle, FALSE);
			chal_audiomic_SetAuxBiasContinuous( mic_handle, FALSE);
			micPoweredOn[ AUDDRV_MIC_AUX ]= FALSE;
		}
	}
	...
	if ( auddrv_mic_not_used( AUDDRV_MIC_DIGI1 ) && micPoweredOn[AUDDRV_MIC_DIGI1]== TRUE
		...
		&& auddrv_mic_not_used( AUDDRV_DUAL_MIC_ANALOG_DIGI2 ) && micPoweredOn[AUDDRV_DUAL_MIC_ANALOG_DIGI2]== TRUE
		)
	{
		...
		micPoweredOn[ AUDDRV_DUAL_MIC_ANALOG_DIGI2 ]= FALSE;
	}
}
