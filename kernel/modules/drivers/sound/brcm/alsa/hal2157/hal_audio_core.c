...
#ifdef ANDROID_FEATURE_VOICE_RECORDING
// flag will be set/clear when vpu recording is on/off
// used to protect the vpu path when voice call is disabled
static Boolean audio_isVoiceRecordingOn = FALSE;
#endif
...
//-- BRCM add CSP#252841
		...
		case AUDIO_ID_VPU_RECORD:
			...
			if (!bCur_StatusMicMute)
				HAL_AUDIO_UnmuteMicrophone();
#ifdef ANDROID_FEATURE_VOICE_RECORDING
			audio_isVoiceRecordingOn = TRUE;
        //printk(KERN_ALERT "HW Enable path - audio_isVoiceRecordingOn: %d\n",audio_isVoiceRecordingOn);
#endif
			break;
...
#endif
			if(!audio_isVoiceOn)
				HAL_AUDIO_SetAudioMode( AUDIO_MODE_NUMBER+(AudioMode_t)pathCfgPtr->outputChnl );
...
static void HAL_AUDIO_DisablePath( HAL_AUDIO_Param_PathCfg_t *pathCfgPtr )
{
	...
	switch(pathCfgPtr->audioID)
		...
		case AUDIO_ID_CALL16K:
#ifdef ANDROID_FEATURE_VOICE_RECORDING
								// do not turn uplink off is voice recording is ongoing
								// can not do much and have to follow dsp call sequence
			//printk(KERN_ALERT "HW disable path - audio_isVoiceRecordingOn: %d\n",audio_isVoiceRecordingOn);					
			if (audio_isVoiceRecordingOn == FALSE)
#endif						
				AUDIO_DRV_ConnectMicrophoneUplink( FALSE );
			AUDIO_DRV_Set_DL_OnOff( FALSE );
	...
	case AUDIO_ID_VPU_VT_LOOPBACK:
		...
		case AUDIO_ID_VPU_RECORD:
#ifdef ANDROID_FEATURE_VOICE_RECORDING
		//printk(KERN_ALERT "HW disable path ID_VPU - audio_isVoiceRecordingOn: %d\n",audio_isVoiceRecordingOn);
			audio_isVoiceRecordingOn = FALSE;
#endif						
...
static void HAL_AUDIO_SetTuneParam (HAL_AUDIO_Param_Tune_t *tune)
{
	switch (tune->paramType)
	{
		...
		case HAL_AUDIO_TUNE_PARAM_EXTAMP_PREPGA:
			audioParam=AudioExtDev_GetVolume();
			tune->paramValue = audioParam;
			break;
