      ...
      case AUDIO_MODE_SPEAKERPHONE:
		...
		case AUDIO_MODE_HEADSET:
      	...
      	case AUDIO_MODE_RESERVE:
			return NCP2704_OUTPUT_HEADSET_AND_LOUDSPK;
		...
		if(bStatusVoice == AUDIO_OFF)
		{
			...
			if (bStatusPcmBack == AUDIO_OFF)
			{
				AUDIO_DRV_Set_DL_OnOff(AUDIO_OFF);
	  		   ...
	  		   // do not disable audio if voice recording is ongoing 
	  		   //printk(KERN_ALERT "Audio drv set voicepath - bStatusInputPath: %d\n",bStatusInputPath);
			  if (bStatusInputPath == AUDIO_OFF)
  #endif
...
void AUDIO_DRV_SetMicrophonePathOnOff(AudioMode_t voiceMode , Boolean on_off)
{
	...
	//printk(KERN_ALERT "Set MIC path on off - bStatusInputPath: %d\n",bStatusInputPath); 
