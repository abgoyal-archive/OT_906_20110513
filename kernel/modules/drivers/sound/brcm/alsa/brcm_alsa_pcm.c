...
//pcm16kcapture
extern int 	PCM16kCaptureStop(struct snd_pcm_substream * substream);
extern int 	PCM16kCaptureStart(struct snd_pcm_substream * substream, VPUDumpFramesCB_t pfDumpFrames);
...
#define	PCM_MAX_CAPTURE_BUF_BYTES 		(20*320) //(8*1024) // = (8000*2)		// one second data
...
int	SwitchPCMOutputDevice(int subdevice)
{
	...
	prevDev = p_dev_channel->outputdevice;
	prevID = sgTableIDChannelOfDev[prevDev].clientID;

	
	if(prevID != audioControlParam.param_pathCfg.audioID)
	{
		//if ID changed,
		//and previous ID is AUDIO_ID_CALL, disable DisableVoiceCall4Dev
		if(prevID == AUDIO_ID_CALL)//CALL->IDLE
			DisableVoiceCall4Dev(prevDev);	
		else //IDLE --> CALL
			EnableVoiceCall4Dev(subdevice);
	}
	else
		EnableVoiceCall4Dev(subdevice);

	if (snd_pcm_running(substream))
		EnableAudioPath4Dev(subdevice); 	
	p_dev_channel->outputdevice = subdevice;
...
)
{
	...
	dev->outputdevice = substream->number;
...
static int PcmPlaybackClose(struct snd_pcm_substream * substream)
{
	...
	DisableVoiceCall4Dev(p_dev_channel->outputdevice);
...
//	DEBUG("\n%lx:playback_prepare rate=%d, format=%d, channels=%d subdevice=%d\n",jiffies, runtime->rate, runtime->format, runtime->channels, substream->number);
	...
	EnableVoiceCall4Dev(p_dev_channel->outputdevice);
	...
	switch (cmd) 
	{
		case SNDRV_PCM_TRIGGER_START:
			/* do something to start the PCM engine */
			{
				...
				EnableAudioPath4Dev(p_dev_channel->outputdevice);
		...
		case SNDRV_PCM_TRIGGER_STOP:
			...
			DisableAudioPath4Dev(p_dev_channel->outputdevice);
			break;
...
//	DEBUG("\n %lx:CBDumpFrames appl_ptr=%d pSrc=0x%x, nFrames=%d pre_dstIdx=%d\n",jiffies, runtime->control->appl_ptr, (unsigned int)pSrc, (int)nFrames,dstIdx);
	...
	if(0==g_brcm_alsa_chip->pcm_read_ptr[1] && runtime->rate==8000) //zero the first frame
	{
		memset(pSrc, 0, bytes);
//		DEBUG("\n %lx:CBDumpFrames appl_ptr=%d pSrc=0x%x, nFrames=%d pre_dstIdx=%d\n",jiffies, runtime->control->appl_ptr, (unsigned int)pSrc, (int)nFrames,dstIdx);
	}
...
int 	VPMCaptureStart(struct snd_pcm_substream * substream)
{
	...
	//Enable path for recording: AUDIO_MODE_RECORD
	{
		...
		else if (substream->number == 0)
			audioControlParam.param_pathCfg.outputChnl = AUDIO_CHNL_HANDSET;
...
)
{
	...
	switch (cmd) 
	{
		...
		VPMCaptureStart(substream);
			...
			PCM16kCaptureStart(substream, CBDumpFrames);
...
int __devinit PcmDeviceNew(struct snd_card *card)
{
	...
	err=snd_pcm_lib_preallocate_pages(substream, SNDRV_DMA_TYPE_CONTINUOUS, snd_dma_continuous_data(GFP_KERNEL),
                                     PCM_MAX__PLAYBACK_BUF_BYTES, PCM_MAX__PLAYBACK_BUF_BYTES);
	if(err)
	...
	err=snd_pcm_lib_preallocate_pages(substream, SNDRV_DMA_TYPE_CONTINUOUS, snd_dma_continuous_data(GFP_KERNEL),
                                     PCM_MAX_CAPTURE_BUF_BYTES, PCM_MAX_CAPTURE_BUF_BYTES);
	if(err)
