...
int AudioExtDev_GetVolume(void)
{
    UInt8 extVol;
	...
	switch(AudioExtDev_GetOutput()) {
		...
		case NCP2704_OUTPUT_HANDSET:
			extVol =ncp2704_regs[NCP2704_SPEAKER_VOL];
			break;
		case NCP2704_OUTPUT_HEADSET:
			extVol=ncp2704_regs[NCP2704_LEFT_HEADSET_VOL];
			break;
		default:
			break;
	}
	mutex_unlock(&amp_dev->ncp2704_lock);
	pr_info("NCP2704:AudioExtDev_GetVolume extVol=%d  \r\n",extVol);
	return extVol;
}
EXPORT_SYMBOL(AudioExtDev_GetVolume);

void AudioExtDev_SetVolume(UInt8 extVol)
{

	if (extVol > NCP2704_SPKVOL_P12DB)
		extVol = NCP2704_SPKVOL_P12DB;
	...
	switch(AudioExtDev_GetOutput()) {
		...
		case NCP2704_OUTPUT_HEADSET:
			...
			break;
       case NCP2704_OUTPUT_HEADSET_AND_LOUDSPK:
			ncp2704_regs[NCP2704_SPEAKER_VOL] = extVol;
			ncp2704_regs[NCP2704_LEFT_HEADSET_VOL] = extVol;
			ncp2704_regs[NCP2704_RIGHT_HEADSET_VOL] = extVol;
			break;
...
void AudioExtDev_PowerOn(AudioMode_t audio_mode)
{
   ...
   case NCP2704_OUTPUT_HEADSET_AND_LOUDSPK:
		in_val &= ~(NCP2704_IN1G | NCP2704_IN2G);;
		out_val &= ~(NCP2704_OUTMODE_MASK);
		out_val |= (NCP2704_SD | NCP2704_OUTMODE(AMMP_OUT_IN1A_IN1B_SPK_IN1A_LEFT_HP_IN1B_RIGHT_HP));
		break;
