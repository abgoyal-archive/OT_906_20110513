             ...
             unsigned int cmd, UInt32 arg)
{
	...
	case PCG_IOCTL_GetExtPAVol:
	{
		UInt32 extvol;
		HAL_AUDIO_Param_Tune_t tuneCfg = {0};

		tuneCfg.paramType = HAL_AUDIO_TUNE_PARAM_EXTAMP_PREPGA;
		if ((rtn = HAL_AUDIO_Ctrl_Blocking(ACTION_AUD_TUNE, &tuneCfg, NULL)) !=0)
			rtn = -1;

		extvol=tuneCfg.paramValue;
		copy_to_user( (UInt32 *)arg, &extvol, sizeof(UInt32));
		pr_info("AUD:  PCG_IOCTL_GetExtPAVol	extvol=%d  \r\n", extvol);
