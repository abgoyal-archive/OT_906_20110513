...
static int gememalloc_release(GEMEMALLOC_HDL hdl)
{
	...
	// delete remaining nodes, add memory sizes of all nodes to head node
	while(curr)
	{
		chunk* tmpCurr = curr;
		head->size += curr->size;
	...
	printk("gememalloc_release OK: size: 0x%08x, addr: 0x%08x\n", head->size, head->address);
				...
				unsigned int phy_start_address)
{
    ...
    memhead->prev = NULL;
    memhead->next = NULL;
	printk("gememalloc_logic_init Values: size: 0x%08x, addr: 0x%08x\n", memory_size, phy_start_address);
	return 0;
}
...
static int FreeMemory(GEMEMALLOC_HDL hdl, unsigned long *busaddr)
{
    ...
    if(head != NULL)
		{
		printk("GEMEMFREE OK: size: 0x%08x, addr: 0x%08x\n", head->size, head->address);
		head->used = false;
		if(head->prev != NULL )
			{
