...
)
{
	...
	{
#ifdef UNDER_LINUX
    // Use kmalloc instead of malloc in Linux platform
		LocalData = kmalloc ((LocalDescriptorSize * NumberOfBuffers), GFP_KERNEL);
#else
		LocalData = malloc (LocalDescriptorSize * NumberOfBuffers);
#endif //UNDER_LINUX

		if (!LocalData)
		{
			IPC_TRACE (IPC_Channel_Error, "IPC_CreateBufferPool", "LocalData malloc Failed", 0, 0, 0, 0);
			return 0;
		}
	} else {
		LocalData = 0;
...
IPC_Buffer IPC_AllocateBuffer (IPC_BufferPool Pool)
{
	...
	IPC_TRACE (IPC_Channel_Pool, "IPC_AllocateBuffer", "Buf %d (%08X) Pool %08X, %d Left",
		IPC_BufferId  (Buffer),
		Buffer,
		Pool,
		BufferCount);
...
IPC_Buffer IPC_AllocateBufferWait (IPC_BufferPool Pool, IPC_U32 MilliSeconds)
{
	...
	IPC_TRACE (IPC_Channel_FlowControl, "IPC_AllocateBufferWait", "Pool %08X Empty, waiting for %d Milliseconds", Pool, MilliSeconds, 0, 0);

	if (IPC_ERROR == IPC_EVENT_WAIT (PoolPtr->EmptyEvent, MilliSeconds))
	{
