...
#define BCMLOG_MAX_LOG_ID		8191		///<	maximum LOG ID
 ...
 **/
typedef enum {
	BCMLOG_CPCRASH_MTD,			///<	write to MTD (flash partition)
	BCMLOG_CPCRASH_SDCARD,		///<	write to SDCARD
	BCMLOG_CPCRASH_BMTT			///<	send to MTT
}	BCMLOG_CpCrashDumpDevice_t ;


/**
 *	Android logcat dump target device
 **/
typedef enum {
	BCMLOG_LOGCAT_MTD,			///<	write to flash: /data/brcm/logcat/YYMMDDHHMMSS_reason/
	BCMLOG_LOGCAT_SDCARD,		///<	write to SDCARD: /sdcard/logcat/YYMMDDHHMMSS_reason/
	BCMLOG_LOGCAT_DISABLED			/// <	disable the logcat dump
}	BCMLOG_LogcatDumpDevice_t ;

/**
 *	Runtime log target device
 **/
typedef enum {
	BCMLOG_RUNLOG_UNDEFINED,		
	BCMLOG_RUNLOG_SDCARD,	
	BCMLOG_RUNLOG_FLASH,
	BCMLOG_RUNLOG_BMTT
}	BCMLOG_RunlogDevice_t ;
