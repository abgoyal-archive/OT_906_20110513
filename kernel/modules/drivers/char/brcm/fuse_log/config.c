...
{	
	...
	BCMLOG_LogcatDumpDevice_t old_logcat_dump_dev = g_config.logcat_dump_dev ;
	BCMLOG_LogcatDumpDevice_t new_logcat_dump_dev = old_logcat_dump_dev ;
	
	BCMLOG_RunlogDevice_t old_runlog_dev = g_config.runlog_dev ;
	BCMLOG_RunlogDevice_t new_runlog_dev = old_runlog_dev ;
	...
	else if( FindTkn( buffer, "logcatdump=sdcard" ) )
		new_logcat_dump_dev = BCMLOG_LOGCAT_SDCARD ;
	else if( FindTkn( buffer, "logcatdump=disabled" ) )
		new_logcat_dump_dev = BCMLOG_LOGCAT_DISABLED ;
	else if( FindTkn( buffer, "logcatdump=mtd" ) )
		new_logcat_dump_dev = BCMLOG_LOGCAT_MTD ;
	else if( FindTkn( buffer, "runlog=bmtt" ) )
		new_runlog_dev = BCMLOG_RUNLOG_BMTT ;
	else if( FindTkn( buffer, "runlog=sdcard" ) )
		new_runlog_dev = BCMLOG_RUNLOG_SDCARD ;
	else if( FindTkn( buffer, "runlog=flash" ) ) 
		new_runlog_dev = BCMLOG_RUNLOG_FLASH ;
	...
	else if( new_logcat_dump_dev != old_logcat_dump_dev )
	{
		g_config.logcat_dump_dev = new_logcat_dump_dev ;
		printk( "BCMLOG: LOGCAT DUMP CONFIGURATION CHANGED FROM %d TO %d\n", (int)old_logcat_dump_dev, (int)new_logcat_dump_dev ) ;
		BCMLOG_SaveConfig( 1 ) ;
	}
	else if( new_runlog_dev != old_runlog_dev )
	{
		g_config.runlog_dev = new_runlog_dev ;
		printk( "BCMLOG: RUN LOG CONFIGURATION CHANGED FROM %d TO %d\n", (int)old_runlog_dev, (int)new_runlog_dev ) ;
		BCMLOG_SaveConfig( 1 ) ;
	}
...
static void try_load_config( struct work_struct *work )
{
	...
	/*
	 *	if unable to read configuration set runtime logging to MTT
	 */
	else 
	{
		g_config.runlog_dev = BCMLOG_RUNLOG_BMTT ;
	}
...
void BCMLOG_InitConfig( void ) 
{
	...
	/*
	 *	set default LOGCAT dump disabled
	 */
	g_config.logcat_dump_dev = BCMLOG_LOGCAT_DISABLED ;

	/*
	 *	set runtime log device 'undefined'
	 */
	g_config.runlog_dev = BCMLOG_RUNLOG_UNDEFINED ;
...
BCMLOG_RunlogDevice_t BCMLOG_GetRunlogDevice( void ) 
{
