...
#else //CONFIG_BRCM_FUSE_RIL_CIB
    if (rsp->result != SIMACCESS_SUCCESS)
    {
        KRIL_DEBUG(DBG_ERROR,"Get Block status failed!! result:%d\n", rsp->result);
        return 0;
    }
    else
    {
...
void UpdateFDNRecordRequest(KRIL_CmdList_t *pdata)
{
    ...
    switch (alpha_coding)
    {
        ...
        default:
            while ((cmddata[i] != SIM_RAW_EMPTY_VALUE) && (i < alpha_id_num) && (i < sizeof(alpha)))
            {
                alpha[i] = cmddata[i];
                i++;
            }
...
#else
            CAPI2_SIM_IsPUKBlocked(GetNewTID(), GetClientID(), SI_CHV2);
#endif
...
#else
            CAPI2_SIM_IsPINBlocked(GetNewTID(), GetClientID(), SI_CHV2);
#endif
...
void KRIL_SimIOHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch (pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            ...
            // For some EF in USIM, Android RIL framework give wrong path.
            // Modify the path in KRIL. 
            if (SIM_APPL_3G == KRIL_GetSimAppType() &&
