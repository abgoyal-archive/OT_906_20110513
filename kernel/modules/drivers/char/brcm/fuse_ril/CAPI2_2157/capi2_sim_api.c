...
void CAPI2_SIM_SubmitMulRecordEFileReq( UInt32 tid, UInt8 clientID, UInt8 socketId, APDUFileID_t efileId, APDUFileID_t dfileId, UInt8 firstRecNum, 
					UInt8 numOfRec, UInt8 recLen, UInt8 pathLen, const UInt16 *pSelectPath, Boolean optimizationFlag1, Boolean optimizationFlag2 )
{
	CAPI2_ReqRep_t req;
	char* stream;
	UInt32 len;
	...
	/* message specific */
	req.req_rep_u.sim_mul_record_req.socketId = socketId;
	req.req_rep_u.sim_mul_record_req.efileId = efileId;
	req.req_rep_u.sim_mul_record_req.dfileId = dfileId;
	req.req_rep_u.sim_mul_record_req.firstRecNum = firstRecNum;
	req.req_rep_u.sim_mul_record_req.numOfRec = numOfRec;
	req.req_rep_u.sim_mul_record_req.recLen = recLen;
	req.req_rep_u.sim_mul_record_req.pathLen = pathLen;
	req.req_rep_u.sim_mul_record_req.pSelectPath = (UInt16*) pSelectPath;
	req.req_rep_u.sim_mul_record_req.optimizationFlag1 = optimizationFlag1;
	req.req_rep_u.sim_mul_record_req.optimizationFlag2 = optimizationFlag2;

	req.respId = MSG_SIM_MUL_REC_DATA_RSP;

	/* serialize */
	CAPI2_SerializeReqRsp(&req, tid, MSG_SIM_MUL_REC_DATA_REQ, clientID, result, &stream, &len);
