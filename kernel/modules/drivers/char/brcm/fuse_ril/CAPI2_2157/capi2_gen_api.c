...
void CAPI2_SYS_CellLock(UInt32 tid, UInt8 clientID, Boolean Enable, BandSelect_t lockBand, UInt8 lockrat, UInt16 lockuarfcn, UInt16 lockpsc)
{
	CAPI2_ReqRep_t req;
	char* stream;
	UInt32 len;
	...
	req.req_rep_u.CAPI2_SYS_CellLock_Req.Enable = Enable;
	req.req_rep_u.CAPI2_SYS_CellLock_Req.lockBand = lockBand;
	req.req_rep_u.CAPI2_SYS_CellLock_Req.lockrat = lockrat;
	req.req_rep_u.CAPI2_SYS_CellLock_Req.lockuarfcn = lockuarfcn;
	req.req_rep_u.CAPI2_SYS_CellLock_Req.lockpsc = lockpsc;
	req.respId = MSG_MS_CELL_LOCK_RSP;
	CAPI2_SerializeReqRsp(&req, tid, MSG_MS_CELL_LOCK_REQ, (UInt8)clientID, result, &stream, &len);
#ifdef BYPASS_IPC
	CAPI2_DeserializeAndHandleReq(stream, len);
#endif
}
...
void CAPI2_SYSPARM_GetImei(UInt32 tid, UInt8 clientID, IMEI_TYPE_t imei_type)
{
	...
	req.req_rep_u.CAPI2_SYSPARM_GetImei_Req.imei_type = imei_type;
...
void CAPI2_SIM_GetUsimCarrierMode(UInt32 tid, UInt8 clientID)
{
	CAPI2_ReqRep_t req;
	char* stream;
	UInt32 len;
	...
	req.respId = MSG_SIM_GET_USIM_CARRIER_MODE_RSP;
	CAPI2_SerializeReqRsp(&req, tid, MSG_SIM_GET_USIM_CARRIER_MODE_REQ, (UInt8)clientID, result, &stream, &len);
#ifdef BYPASS_IPC
	CAPI2_DeserializeAndHandleReq(stream, len);
#endif
}
...
void CAPI2_AUDIO_DRV_SetAudioMode(UInt32 tid, UInt8 clientID, AudioMode_t mode, UInt8 op)
{
	CAPI2_ReqRep_t req;
	char* stream;
	UInt32 len;
	...
	req.req_rep_u.CAPI2_AUDIO_DRV_SetAudioMode_Req.mode = mode;
	req.req_rep_u.CAPI2_AUDIO_DRV_SetAudioMode_Req.op = op;
	req.respId = MSG_AUDIO_DRV_SetAudioMode_RSP;
	CAPI2_SerializeReqRsp(&req, tid, MSG_AUDIO_DRV_SetAudioMode_REQ, (UInt8)clientID, result, &stream, &len);
#ifdef BYPASS_IPC
	CAPI2_DeserializeAndHandleReq(stream, len);
#endif
}
...
void CAPI2_AUDIO_GetSettingsEx(UInt32 tid, UInt8 clientID, UInt8 audioMode, UInt8 audioOp)
{
	CAPI2_ReqRep_t req;
	char* stream;
	UInt32 len;
	...
	req.req_rep_u.CAPI2_AUDIO_GetSettingsEx_Req.audioMode = audioMode;
	req.req_rep_u.CAPI2_AUDIO_GetSettingsEx_Req.audioOp = audioOp;
	req.respId = MSG_AUDIO_GET_SETTINGSEX_RSP;
	CAPI2_SerializeReqRsp(&req, tid, MSG_AUDIO_GET_SETTINGSEX_REQ, (UInt8)clientID, result, &stream, &len);
#ifdef BYPASS_IPC
	CAPI2_DeserializeAndHandleReq(stream, len);
#endif
}
...
void CAPI2_SMS_WriteSMSPduToSIMRecordReq(UInt32 tid, UInt8 clientID, UInt8 length, UInt8 *inSmsPdu, SIMSMSMesgStatus_t smsState, UInt16 rec_no)
{
	CAPI2_ReqRep_t req;
	char* stream;
	UInt32 len;
...
void CAPI2_SIM_IsTestSIM(UInt32 tid, UInt8 clientID)
{
	...
	req.respId = MSG_SIM_ISTESTSIM_RSP;
	CAPI2_SerializeReqRsp(&req, tid, MSG_SIM_ISTESTSIM_REQ, (UInt8)clientID, result, &stream, &len);
#ifdef BYPASS_IPC
	CAPI2_DeserializeAndHandleReq(stream, len);
#endif
}

void CAPI2_OEM_Set_Data(UInt32 tid, UInt8 clientID, UInt32 type, CAPI2_Oem_Payload_t *payload)
{
	CAPI2_ReqRep_t req;
	char* stream;
	UInt32 len;
	...
	req.req_rep_u.CAPI2_OEM_Set_Data_Req.type = type;
	req.req_rep_u.CAPI2_OEM_Set_Data_Req.payload = payload;
	req.respId = MSG_MS_OEM_SET_DATA_RSP;
	CAPI2_SerializeReqRsp(&req, tid, MSG_MS_OEM_SET_DATA_REQ, (UInt8)clientID, result, &stream, &len);
#ifdef BYPASS_IPC
	CAPI2_DeserializeAndHandleReq(stream, len);
#endif
}

void CAPI2_OEM_Get_Data(UInt32 tid, UInt8 clientID, UInt32 type)
{
	CAPI2_ReqRep_t req;
	char* stream;
	UInt32 len;
	...
	req.req_rep_u.CAPI2_OEM_Get_Data_Req.type = type;
	req.respId = MSG_MS_OEM_GET_DATA_RSP;
	CAPI2_SerializeReqRsp(&req, tid, MSG_MS_OEM_GET_DATA_REQ, (UInt8)clientID, result, &stream, &len);
#ifdef BYPASS_IPC
	CAPI2_DeserializeAndHandleReq(stream, len);
#endif
}

void CAPI2_WL_PsSetFilterList(UInt32 tid, UInt8 clientID, ClientInfo_t *inClientInfoPtr, UInt8 cid, WL_SocketFilterList_t *inDataPtr)
{
	CAPI2_ReqRep_t req;
	char* stream;
	UInt32 len;
	...
	req.req_rep_u.CAPI2_WL_PsSetFilterList_Req.inClientInfoPtr = inClientInfoPtr;
	...
	CAPI2_SerializeReqRsp(&req, tid, MSG_WL_PS_SET_FILTER_LIST_REQ, (UInt8)clientID, result, &stream, &len);
#ifdef BYPASS_IPC
	CAPI2_DeserializeAndHandleReq(stream, len);
#endif
}
