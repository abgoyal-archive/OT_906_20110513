...
XDR_ENUM_FUNC(USIM_CARRIER_MODE_t)
...
bool_t 
xdr_SIM_MUL_REC_DATA_t(XDR* xdrs, SIM_MUL_REC_DATA_t* data)
{
	XDR_LOG(xdrs,"SIM_MUL_REC_DATA_t")

	if( XDR_ENUM(xdrs, &data->result, SIMAccess_t) &&
		_xdr_u_char(xdrs, &data->num_of_rec, "num_of_rec") && 
		_xdr_u_char(xdrs, &data->rec_len, "rec_len") )
	{
		u_int len = (u_int) (data->num_of_rec * data->rec_len);
		return xdr_bytes(xdrs, (char **)(void*)&data->rec_data, &len, MAX_MUL_REC_DATA_SIZE);
	}
...
bool_t 
xdr_CAPI2_SimMulRecordReq_t(XDR* xdrs, CAPI2_SimMulRecordReq_t* data)
{
	bool_t result;
	XDR_LOG(xdrs,"CAPI2_SimMulRecordReq_t")

	if ( _xdr_u_char(xdrs, &data->socketId, "socketId") &&
		 XDR_ENUM(xdrs, &data->efileId, APDUFileID_t) &&
		 XDR_ENUM(xdrs, &data->dfileId, APDUFileID_t) &&
		 _xdr_u_char(xdrs, &data->firstRecNum, "firstRecNum") &&
		 _xdr_u_char(xdrs, &data->numOfRec, "numOfRec") &&
		 _xdr_u_char(xdrs, &data->recLen, "recLen") &&
		 _xdr_u_char(xdrs, &data->pathLen, "pathLen") &&
		 xdr_Boolean(xdrs, &data->optimizationFlag1) &&
		 xdr_Boolean(xdrs, &data->optimizationFlag2) )
	{		 
		u_int len = (u_int) data->pathLen;
			
		result = xdr_array(xdrs, (char **)(void*) &data->pSelectPath, &len, ~0, sizeof(UInt16), (xdrproc_t) xdr_u_int16_t);
	}
	else
	{
		result = FALSE;
...
bool_t 
xdr_SIM_GET_CARRIER_MODE_RSP_t(XDR* xdrs, SIM_GET_CARRIER_MODE_RSP_t* info)
{
	XDR_LOG(xdrs,"SIM_GET_CARRIER_MODE_RSP_t")

    if( XDR_ENUM(xdrs, &info->carrierMode, USIM_CARRIER_MODE_t) 
		)
		return(TRUE);
	else
		return(FALSE);
