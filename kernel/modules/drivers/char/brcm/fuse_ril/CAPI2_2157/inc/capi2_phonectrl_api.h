...
#define MAX_OEM_BUFFER_SIZE	200

/**
CAPI OEM Paylaod info
**/
typedef struct
{
	UInt8 buffer[MAX_OEM_BUFFER_SIZE];						///< Paylaod Buffer. If unused then set to NULL
	UInt32 len;							///< If buffer is NULL thenm set len tp zero else size of buffer
	UInt32 param;						///< Param value.
}CAPI2_Oem_Payload_t;
...
typedef struct
{
	Boolean fromGmmInfo;		//TRUE: this LSA ID is extracted from the GMM Information message, 
								//FALSE: this LSA Id is extracted from the MM Information Message 
	UInt8	lsaLength;			//As per 3GPP 24.008 Sec 10.5.3.11 a length of 0 is used to indicate that the 
								//MS has moved to an area where there is no LSA available for that MS. Othewise it is 3 octets
	UInt8	lsaArray[3];		//LSA Id is 24 bits in length. Bit 0 indicates if the LSA is a PLMN Significant number or universal
								//identity (3GPP 23.003 Sec 11)							
...
typedef enum
{
	...
	TIMEZONE_UPDATEMODE_NO_TZ_UPDATE	///< Date/Time without adjusted timezoe
...
void CAPI2_AUDIO_ASIC_SetAudioModeEx(UInt32 tid, UInt8 clientID, AudioMode_t mode, UInt8 op);
