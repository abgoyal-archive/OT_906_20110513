...
static char banner[] __initdata = KERN_INFO "Camera Driver: 1.00 (built on "__DATE__" "__TIME__")\n";
...
static UInt32			sCaptureStillSize;				// Actual byte count of Still image
...
static HAL_CAM_Result_en_t camDmaCreateList( int line_bytes, int lines, DMADRV_LLITABLE_MODE_t mode );
static HAL_CAM_Result_en_t CAM_UpdateParm( CAM_Parm_t *parmp );
static inline void checkCameraID(CamSensorSelect_t sensor);
static int cam_i2c_command(struct i2c_client *device, unsigned int cmd, void *arg);
static void writeFrame( void *bufferp, void *arg );
static int processFrame( void *unused );
static int cam_pm_update (PM_CompPowerLevel compPowerLevel, PM_PowerLevel sysPowerLevel);
...
static HAL_CAM_Result_en_t Cam_SensorIntfSeqSel(CamSensorSelect_t nSensor, CamSensorSeqSel_t nSeqSel);
static HAL_CAM_Result_en_t Cam_SensorCntrlSeq(CamSensorIntfCntrl_st_t *seq, UInt32 length);
static HAL_CAM_Result_en_t camSetUpCapture( CamSensorSelect_t Sensor );

// Camera Baseband Configuration -----------------------------------------
static HAL_CAM_Result_en_t Cam_InitVsyncIrq( CamIntfConfig_st_t *pSyncIntfConfig );
static HAL_CAM_Result_en_t Cam_CfgImageCapture(CamImageSize_t resolution, CamCaptureMode_t mode, CamSensorSelect_t sensor);
static HAL_CAM_Result_en_t Cam_CfgWindowScale(CamWindowConfig_st_t *pcam_window_config);
static HAL_CAM_Result_en_t Cam_CfgDma(CamWindowConfig_st_t *pcam_window_config, DMADRV_LLITABLE_MODE_t mode, Boolean repeat );
static HAL_CAM_Result_en_t Cam_ConfigCamSensorZoom(Boolean forPreview, CamSensorSelect_t nSensor);

// Camera Controller Function Prototypes -----------------------------------------
static HAL_CAM_Result_en_t Cam_CfgCmiController( CamSensorSeqSel_t mode, CamIntfConfig_st_t *pCmiIntfConfig );
// I2C Configuration -----------------------------------------
...
{
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_FUNCTION, "cam_i2c_attach_adapter start\n");
        cam_i2c_datap = kmalloc( sizeof( *cam_i2c_datap ), GFP_KERNEL );
        memset( cam_i2c_datap, 0, sizeof( *cam_i2c_datap ));

        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_FUNCTION, "cam_i2c_attach_adapter successful\n");
...
static void PushQueue(struct BufQueue* queue,CAM_BufData* buf)
{
  ...
  if (queue->Num == MAX_QUEUE_SIZE)  
	{
		CAM_DEBUG(DBG_INFO,( "Push datat fail!!: Camera  Data Queue Full!\n"));
	}
	else
	{
		CAM_DEBUG(DBG_TRACE,("PushQueue Index %d  Address 0x%x\n",buf->id ,buf->busAddress));
...
static bool  PullQueue(struct BufQueue* queue,CAM_BufData* buf)
{
	...
	if(queue->isWaitQueue) return false;
	spin_lock_irqsave(&queue->lock,stat);
  if (queue->Num == 0) 
	{
		CAM_DEBUG(DBG_INFO,( "Pull Queue failed!! Camera  Data Queue  Emptyl!\n" ));
		buf->busAddress = NULL;
		buf->id = -1;
	}
	else
    ...
    queue->Num--; 		
		CAM_DEBUG(DBG_TRACE,(" PullQueue Index %d  Address 0x%x\n",buf->id ,buf->busAddress));
	}
	spin_unlock_irqrestore(&queue->lock,stat);
	return true;
}
...
static void WakeupPushQueue(struct BufQueue* queue,CAM_BufData* buf)
{
  ...
  if (queue->Num == MAX_QUEUE_SIZE) 
	{
		CAM_DEBUG(DBG_INFO,( "Wake Push datat failed!!l Camera  Data Queue Full!\n"));
	}
	else
	{
		CAM_DEBUG(DBG_TRACE,(" WakeupPushQueue Index %d  Address 0x%x\n",buf->id ,buf->busAddress));
		queue->data[queue->WriteIndex] . busAddress =buf->busAddress ;
		queue->data[queue->WriteIndex] . id =buf->id ;
  ...
  if (queue->Num == 0)  
	{
		buf->busAddress = NULL;
		buf->id = -1;
		CAM_DEBUG(DBG_INFO,( "Wait WaitPullQueue failed !! Camera  Data Queue Empty!\n" ));
	}
	else
	{
		CAM_DEBUG(DBG_TRACE,(" WaitPullQueue Index %d  Address 0x%x\n",buf->id ,buf->busAddress));
		buf->busAddress= queue->data[queue->ReadIndex] . busAddress ;
		buf->id =  queue->data[queue->ReadIndex] . id ;
		buf->timestamp=  queue->data[queue->ReadIndex] . timestamp;     
		queue->ReadIndex = (queue->ReadIndex+1) &MAX_QUEUE_SIZE_MASK;
...
static irqreturn_t dma_isr( void *unused )
{
    ...
    gCamState.dmaCam++;
    CAM_DEBUG(DBG_INFO,( "Camera - dma_isr()\n" ));
    return IRQ_HANDLED;
}
...
static int processFrame( void *unused )
{
	...
	/* A Valid wake-up by VSYNC ISR has gProcessFrameRunning = 1.
         * The disable method wakes up with gProcessFrameRunning = 0 which means exit */
            ...
            if ((sCamDmaMode == DMADRV_STILLS_MODE) && (sCamState == CAM_STATE_PAUSE) && !sCaptureStillSize) {             
                sCaptureStillSize = CAMDRV_GetJpegSize(sCurrentSensor, gDmaBuffer[0].virtPtr);
#if defined(CONFIG_BCM_CAM_STV0986)
                if (sCaptureFrameCountdown >= -15) {
                    if (Cam_MarkerSearch(gDmaBuffer[0].virtPtr)) {
                        sCaptureStillSize = 0;
            ...
            if ( gCamConfig.frameFunc != 0 ) {
                ...
                (*gCamConfig.frameFunc)(ptr, gCamConfig.frameFuncArg);
                //CAM_DEBUG(DBG_INFO,("CAM - processFrame done\n"));
		...
		break;
            ...
            CAM_DEBUG(DBG_INFO,("CAM - processFrame Unknown\n"));
            break;
        }
    }
    return 0;
...
static void writeFrame( void *bufferp, void *arg )
{
    ...
    if (sCamDmaMode != DMADRV_STILLS_MODE) {
        ...
        ptr = (UInt16 *)bufferp;  
        length = (sCaptureStillSize + 1) >> 1; // length in u16
        if (frame->len) {
...
#endif
{
    ...
    if ((sCaptureMode == CamCaptureStill) || (sCaptureMode == CamCaptureStillnThumb)) {
        if (sCamState == CAM_STATE_STILL_CAPTURE_INIT) {
            sCamState = CAM_STATE_STILL_CAPTURE;
            sCaptureStillSize = 0;
        } else if ((sCamState == CAM_STATE_STILL_CAPTURE) && (sCaptureFrameCountdown <= 0)) {
            sCamState = CAM_STATE_PAUSE;
            // sCamDmaMissed = Cam_CheckDmaChan((Int32)gCamConfig.dmaChannel, (UInt32)gDmaBuffer[gDblBufInd].physPtr);
        }else if ((sCamState == CAM_STATE_PAUSE) && !sCaptureStillSize && (atomic_read(&captured) == 0))  {
            sCamState = CAM_STATE_STILL_CAPTURE;
        }
    } else {
...
/* start the next DMA transfer */
	if (sCamState != CAM_STATE_PAUSE) {            
		  if (sCamDmaMode != DMADRV_STILLS_MODE)           
		  {
			...
			{
				CAM_DEBUG(DBG_INFO,( "Camera - Drop frame\n" ));
			}    
		  }
		  else
		  {
    ...
    if ( ((sCamState == CAM_STATE_VIDEO_CAPTURE) || (sCamState == CAM_STATE_PAUSE))
...
//    CAM_DEBUG(DBG_INFO,( "Camera - camStartDmaTransfer()\n" ));
    ...
    dma_setup_chan( gCamConfig.dmaChannel, ptr[0], ptr[1], ptr[2], ptr[3], DMA_FROM_CAM_CONFIG );
}
...
static void camStopDmaTransfer( void )
{
//    CAM_DEBUG(DBG_INFO,( "Camera - camStopDmaTransfer()\n" ));
...
static HAL_CAM_Result_en_t camDmaCreateList( int line_bytes, int lines, DMADRV_LLITABLE_MODE_t mode )
{
    ...
    {
        gCamLinkedList[buffer].sizeInBytes = (lines + 1) * BYTES_PER_LLI;
		gCamLinkedList[buffer].virtPtr = dma_alloc_coherent( NULL,
															 gCamLinkedList[buffer].sizeInBytes, &gCamLinkedList[buffer].physPtr, GFP_KERNEL );

        if ( gCamLinkedList[buffer].virtPtr == NULL )
        {
            CAM_DEBUG(DBG_ERROR,("Camera - Failed to allocate memory ( 0x%08x ) for DMA LL buffer %d\n",
						gCamLinkedList[buffer].sizeInBytes , buffer));
            return HAL_CAM_ERROR_INTERNAL_ERROR;
        }

        ptr = gCamLinkedList[buffer].virtPtr;
        CAM_DEBUG(DBG_INFO,( "Camera - camDmaCreateList(): LLptr=0x%x Destptr=0x%x\n",
				gCamLinkedList[buffer].physPtr,gDmaBuffer[buffer].physPtr ));
        ...
        for (i = 1; i < lines - 1; i++ )
        {
            ...
            ptr[1] = SWAP( gDmaBuffer[buffer].physPtr + line_bytes * j );
            ptr[2] = SWAP( gCamLinkedList[buffer].physPtr + BYTES_PER_LLI * (i + 1) );
        ...
        ptr[2] = 0;
        ptr[3] = SWAP( DMA_FROM_CAM_CONTROL  | transfer_size | REG_DMA_CHAN_CTL_TC_INT_ENABLE );
#else
    ...
    CAM_DEBUG(DBG_INFO,( "Camera - camDmaCreateList(): SrcPtr=0x%x Cntrl=0x%x\n",REG_CAM_DATR_PADDR, (DMA_FROM_CAM_CONTROL | transfer_size)));
...
static int cam_open( struct inode *inode, struct file *file )
{
    ...
    CAM_DEBUG(DBG_INFO,( "Camera - %s called\n", __FUNCTION__ ));
...
// Get Selected Camera Sensor Interface Configuration
    ...
    {
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): CAMDRV_GetIntfConfig(): ERROR \r\n", __FUNCTION__);
...
// Wake Up Camera Device
    ...
    {
        result |= HAL_CAM_ERROR_INTERNAL_ERROR;
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): CAM_Standby(): ERROR: \r\n", __FUNCTION__);
    }
...
// Check Return Code
    ...
    {
        rc = -EFAULT;
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_FUNCTION, "%s(): ERROR \r\n", __FUNCTION__);
    }
...
static ssize_t cam_read( struct file *file, char *buffer, size_t count, loff_t *ppos )
{
    CAM_DEBUG(DBG_INFO,( "cam_read called, major = %d, minor = %d\n", MAJOR( file->f_dentry->d_inode->i_rdev ), MINOR( file->f_dentry->d_inode->i_rdev )));
...
static int cam_release( struct inode *inode, struct file *file )
{
    ...
    CAM_DEBUG(DBG_INFO,( "Camera - %s called\n", __FUNCTION__ ));
    HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_FUNCTION, "%s(): Start \r\n", __FUNCTION__);
...
//this function puts camera in sleep mode.
    ...
    {
        result |= HAL_CAM_ERROR_INTERNAL_ERROR;
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): CAMDRV_SetCamSleep(): ERROR: \r\n", __FUNCTION__);
    }
    else
    {
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_FUNCTION, "%s(): CAMDRV_SetCamSleep() OK \r\n", __FUNCTION__);
    }
...
// Release Vsync IRQ
    ...
    if ( result == HAL_CAM_SUCCESS )
    {
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_FUNCTION, "%s(): End  OK\r\n", __FUNCTION__);
    }
    else
    {
        rc = -EFAULT;
...
static int cam_ioctl( struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg )
{
    ...
    //CAM_DEBUG(DBG_INFO,("cam_ioctl: type: '%c' cmd: 0x%x\n", _IOC_TYPE( cmd ), _IOC_NR( cmd )));
    ...
    switch ( cmd )
    {
        ...
        case CAM_IOCTL_GET_PARAMS:
        {
            ...
            if ( copy_to_user( (CAM_Parm_t *)arg, &parm, sizeof( parm )) != 0 )
                rc = -EFAULT;
        ...
        // Set New Frame Rate if Streaming
            if (gCamConfig.enable) {
                if ( CAMDRV_SetFrameRate( stCamParm.fps, sCurrentSensor ) != HAL_CAM_SUCCESS ) {
                    rc = -EFAULT;
                    HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "CAM_IOCTL_SET_FPS: CAMDRV_SetFrameRate(): ERROR: \r\n");
                }
            }
            break;
        }
        ...
        case CAM_IOCTL_SET_THUMBNAIL_PARAMS:
            if ( copy_from_user( &stThumbnailParm, (CAM_Parm_t *)arg, sizeof( stThumbnailParm )) != 0 )
                rc = -EFAULT;
		...
		case CAM_IOCTL_MEM_REGISTER:
			{
				CAM_BufData phydata;
				if ( copy_from_user( &phydata, (CAM_BufData *)arg, sizeof( CAM_BufData )) != 0 ) {
					rc = -EFAULT;
					break;
				}
				cam_bootmem_offset= 0;
				gCamBootBuffer.physPtr = (dma_addr_t )phydata.busAddress;
			}
			break;
		...
		{
				CAM_BufData buf;
				if ( copy_from_user((void  *) &buf, (const void  *)arg, sizeof( CAM_BufData )) != 0 ) 
				{
					rc = -EFAULT;
				}
				else
				{
					if(gDmaBuffer[buf.id].physPtr == (dma_addr_t)buf.busAddress)    
					{
		...
		case CAM_IOCTL_GET_FRAME:
		{
				...
				if ( copy_to_user( (void*)arg, (const void*)&buf, sizeof( CAM_BufData )) != 0 )
					rc = -EFAULT;
				break;
		}
#if 0
        case CAM_IOCTL_GET_JPEG:
        {
            ...
            gDataReady = 0;
            sCaptureStillSize = CAMDRV_GetJpegSize(sCurrentSensor);    

            length = (sCaptureStillSize + 1) >> 1; // length in u16            
            frame.thumbnailOffset = (length << 1) + 2;
            frame.thumbnailSize     = stThumbnailParm.size_window.end_pixel * stThumbnailParm.size_window.end_line;
            frame.len =  (sCaptureStillSize + 1) >> 1;

            if ( copy_to_user( (CAM_Frame_t *)arg, &frame, sizeof( frame )) != 0 )
                rc = -EFAULT;
...
done_get_frame:
 ...
 /* get single frame */
        case CAM_IOCTL_GET_FRAME1:
        {
            ...
            if ( copy_from_user( &frame, (CAM_Frame1_t *)arg, sizeof( frame )) != 0 ) {
                rc = -EFAULT;
                break;
            }
	    ...
	    /* Invoke processFrame as a regular function
 	     * Blocks on data ready indication by the VSYNC-ISR */
            ...
            if (copy) {
                if ( copy_to_user( frame.buffer, framebuf, frame.len * sizeof( unsigned short )) != 0 )
                    frame.len = 0;
            }
...
done_get_frame:
        ...
        case CAM_IOCTL_GET_JPEG:
        {
            ...
            int length;
            short *fbuf;
            HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_INFO, "CAM_IOCTL_GET_JPEG: sCaptureStillSize=%d\r\n", sCaptureStillSize); 
            ...
            if ( copy_to_user( frame.buffer, fbuf, frame.len * sizeof( unsigned short )) != 0 )
                frame.len = 0;
...
done_get_jpeg:
        ...
        case CAM_IOCTL_GET_THUMBNAIL:
        {
            ...
            length = stThumbnailParm.size_window.end_pixel * stThumbnailParm.size_window.end_line;
            if ( !frame.buffer || (frame.len < length) ) {
                rc = -EPERM;
                goto done_get_thumbnail;
            }
            ...
            thumbnail = CAMDRV_GetThumbnail(framebuf, sCaptureStillSize);

            if ( copy_to_user( frame.buffer, thumbnail, frame.len * sizeof( unsigned short )) != 0 )
                frame.len = 0;
...
done_get_thumbnail:
        ...
        case CAM_IOCTL_GET_STILL_YCbCr:
        {
            ...
            gDataReady = 0;
            sCaptureStillSize = sCamWidth*sCamHeight;			
            frame.len = sCaptureStillSize >>1 ;
...
done_get_YCbCr:
        ...
        case CAM_IOCTL_SET_DIGITAL_EFFECT:
        {
            ...
            if (CAMDRV_SetDigitalEffect(parm.coloreffects, sCurrentSensor) != HAL_CAM_SUCCESS)
               rc = -EFAULT;
	...
	case CAM_IOCTL_SET_SCENE_MODE:
        {
            ...
            if (CAMDRV_SetSceneMode(parm.scenemode, sCurrentSensor)!= HAL_CAM_SUCCESS) {
               rc = -EFAULT;
	...
	case CAM_IOCTL_SET_WB:
        {
            ...
            if (CAMDRV_SetWBMode(parm.wbmode, sCurrentSensor) != HAL_CAM_SUCCESS) {
               rc = -EFAULT;
	...
	case CAM_IOCTL_SET_ZOOM:
		...
		if (CAMDRV_SetZoom(parm.zoom,sCurrentSensor)!= HAL_CAM_SUCCESS) {
               rc = -EFAULT;
               pr_debug("CAM_IOCTL_SET_ZOOM: CAMDRV_SetZoom(): ERROR: \r\n");
            }
        ...
        case CAM_IOCTL_SET_ANTI_BANDING:
        {
            ...
            if ( CAMDRV_SetAntiBanding( parm.antibanding,sCurrentSensor) != HAL_CAM_SUCCESS ) {
                rc = -EFAULT;
	...
	case CAM_IOCTL_SET_FLASH_MODE:
        {
            ...
            if (CAMDRV_SetFlashMode(parm.focus, sCurrentSensor) != HAL_CAM_SUCCESS) {
                rc = -EFAULT;
	...
	case CAM_IOCTL_SET_FOCUS_MODE:
        {
            ...
            if (CAMDRV_SetFocusMode(parm.focus, sCurrentSensor) != HAL_CAM_SUCCESS) {
                rc = -EFAULT;
	...
	case CAM_IOCTL_SET_JPEG_QUALITY:
        {
            ...
            if (CAMDRV_SetJpegQuality(parm.quality, sCurrentSensor)!= HAL_CAM_SUCCESS) {
               rc = -EFAULT;
        ...
        {
            if (CAMDRV_TurnOnAF(sCurrentSensor) != HAL_CAM_SUCCESS) {
                rc = -EFAULT;
        ...
        {
            if (CAMDRV_TurnOffAF(sCurrentSensor) != HAL_CAM_SUCCESS) {
                rc = -EFAULT;
        ...
        case CAM_IOCTL_GET_RESOLUTION_AVAILABLE:
        {
            ...
            CAMDRV_GetResolutionAvailable(parm.size_window.end_pixel,
        ...
        case CAM_IOCTL_READ_REG:
        {
            ...
            if ( copy_from_user( &reg, (CAM_Reg_t *)arg, sizeof( reg )) != 0 ) {
                rc = -EFAULT;
                break;
            }
            ...
            result = CAM_ReadI2c( reg.reg, reg.cnt, (unsigned char *)&reg.val );

            if (result != HAL_CAM_SUCCESS) {
                CAM_DEBUG(DBG_ERROR,("CAM_IOCTL_READ_REG: - I2C Read Error: reg=0x%x  val=0x%x\n", reg.reg, reg.val));
                rc = -EFAULT;
                break;
            }
            ...
            if ( copy_to_user( (CAM_Reg_t *)arg, &reg, sizeof( reg )) != 0 )
                rc = -EFAULT;
        ...
        case CAM_IOCTL_WRITE_REG:
        {
            ...
            result = CAM_WriteI2c( reg.reg, reg.cnt, (u8 *)&reg.val );

            if (result != HAL_CAM_SUCCESS)
                CAM_DEBUG(DBG_ERROR,("CAM_IOCTL_WRITE_REG: - I2C Write Error: reg=0x%x  val=0x%x\n", reg.reg, reg.val));

            break;
        }

        default:
            CAM_DEBUG(DBG_INFO, ("%s() - Unrecognized ioctl: '0x%x'\n", __FUNCTION__, cmd ));
            rc = -ENOTTY;
            break;
    }
...
static ssize_t cam_write( struct file *file, const char *buffer, size_t count, loff_t *ppos )
{
    return -EINVAL;
...
static inline void checkCameraID(CamSensorSelect_t sensor)
{
	...
	printk(KERN_INFO "[CAM] DeviceID = 0x%04x\n", devID);
}
...
static int __init cam_init( void )
{
    ...
    if (( rc = register_chrdev( BCM_CAM_MAJOR, "camera", &cam_fops )) < 0 ) {
        printk( KERN_WARNING "Camera: register_chrdev failed for major %d\n", BCM_CAM_MAJOR );
        return rc;
    }
    ...
    {
        if(!regulator_is_enabled(cam_regulator))
        {
            regulator_enable(cam_regulator);
        }
        #ifdef CONFIG_BCM_CAM_OV2655
        //Change voltage form default 1.3v to 1.5v
        regulator_set_voltage(cam_regulator,1500000,1500000);
        #endif                
    }
#ifdef CONFIG_HAS_WAKELOCK
    ...
    {
        CAM_DEBUG(DBG_INFO,( "Camera - failed to register with power manager\n"));
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): pm_register_component(): ERROR \r\n", __FUNCTION__);
        result |= HAL_CAM_ERROR_INTERNAL_ERROR;
    }
...
// Initialize I2C
    ...
    {
        result |= HAL_CAM_ERROR_INTERNAL_ERROR;
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): Cam_CfgSensorI2c(): ERROR \r\n", __FUNCTION__);
    }
// Configure Camera Controller for Initialization
    if ( Cam_CfgCmiController( SensorInitPwrUp, pSensorIntfConfig ) != HAL_CAM_SUCCESS )
    {
        result |= HAL_CAM_ERROR_INTERNAL_ERROR;
    }
// Init Power-up Sequence for Sensor
    if ( Cam_SensorIntfSeqSel(sCurrentSensor, SensorInitPwrUp) != HAL_CAM_SUCCESS )
    {
        result |= HAL_CAM_ERROR_INTERNAL_ERROR;
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): Cam_SensorIntfSeqSel(): ERROR \r\n", __FUNCTION__);
    }


   CAM_DEBUG(DBG_INFO,( "Camera i2_add_driver - %s called\n", __FUNCTION__ ));
// normal_i2c[0] = sCamI2cDeviceId;
    cam_i2c_datap = NULL;
    rc = i2c_add_driver(&i2c_driver_cam);
//    if ((rc != 0) || (cam_i2c_datap == NULL))
    if (rc != 0) {
        CAM_DEBUG(DBG_ERROR,("Camera - Failed to initialize I2C\n"));
        goto err;
    }
// Supplemental Initialization of Camera Sensor (if needed)
	...
	{
		result |= HAL_CAM_ERROR_INTERNAL_ERROR;
		HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): CAMDRV_Supp_Init(): ERROR \r\n", __FUNCTION__);
	}

// Check Camera Sensor
#ifdef CONFIG_BCM_CAM_OV2655	
	if(CAMDRV_CheckManufacturerID(sCurrentSensor)!=TRUE)
	{
		result |= HAL_CAM_ERROR_INTERNAL_ERROR;
		HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): CAMDRV_CheckManufacturerID(): ERROR: \r\n", __FUNCTION__);
...
// Camera Initialized
    if ( result == HAL_CAM_SUCCESS )
    {
        ...
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_INFO, "%s(): End OK \r\n", __FUNCTION__);
    }
    else
    {
        rc = -EFAULT;
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): Failed \r\n", __FUNCTION__);
    }

 
    HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_INFO, "CLK_CAMCK_ENABLE=0x%x :%s() \r\n",  __REG32(HW_IO_PHYS_TO_VIRT(0x8140004)), __FUNCTION__);
    HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_INFO, "CLK_CAMCK_MODE=0x%x :%s()\r\n", __REG32(HW_IO_PHYS_TO_VIRT(0x8140008)), __FUNCTION__);    
    HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_INFO, "IOCR3=0x%x :%s()\r\n", __REG32(HW_IO_PHYS_TO_VIRT(0x888001C)), __FUNCTION__);
    HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_INFO, "SYSCFG_ANACR10=0x%x :%s()\r\n", __REG32(HW_IO_PHYS_TO_VIRT(0x088800a8)), __FUNCTION__);    


	cam_bootmem_offset = 0; 
#ifdef CONFIG_BCM_CAM_OV2655
    //DVDD,DOVDD and AVDD have to be turned on/off together.Or leak current will happen.  
#else
    if (cam_regulator && !IS_ERR(cam_regulator) && regulator_is_enabled(cam_regulator))
...
static int cam_pm_update(PM_CompPowerLevel compPowerLevel, PM_PowerLevel sysPowerLevel)
{
    ...
    switch (compPowerLevel)
    {
        case PM_COMP_PWR_OFF:
            ...
            {
                rc = -EFAULT;
                HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): CAM_PowerOff(): ERROR: \r\n", __FUNCTION__);
            }
            break;
        ...
        case PM_COMP_PWR_ON:
            ...
            {
                rc = -EFAULT;
                HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): CAM_PowerOn(): ERROR: \r\n", __FUNCTION__);
            }
            break;
    }
   return rc;
...
int camEnable( int level )
{
    ...
    if ( level )
    {
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_FUNCTION, "%s(): TRUE \r\n", __FUNCTION__);
        if ( CAM_Enable( sCurrentSensor ) != HAL_CAM_SUCCESS )
        {
            result |= HAL_CAM_ERROR_INTERNAL_ERROR;
            HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): CAM_Enable(): ERROR \r\n", __FUNCTION__);
        }
    }
    else
    {
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_FUNCTION, "%s: FALSE \r\n", __FUNCTION__);
        if ( CAM_Disable(sCurrentSensor) != HAL_CAM_SUCCESS )
        {
            result |= HAL_CAM_ERROR_INTERNAL_ERROR;
            HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): Cam_Disable():  ERROR: \r\n", __FUNCTION__);
        }
    }
...
// Set up for Image Capture
    ...
    memcpy( &stNewCamWindowConfig, &stCamParm.size_window, sizeof stNewCamWindowConfig );
...
// Set Frames to Discard based on Capture mode
    if ( (sCaptureMode == CamCaptureVideo) || (sCaptureMode == CamCaptureVideonViewFinder) )
    {
...
// Set Capture size & mode
    ...
    if (result != HAL_CAM_SUCCESS)
    {
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s():  Cam_CfgImageCapture():  ERROR: \r\n", __FUNCTION__);
    }
...
int camSetParm( CAM_Parm_t *parmp )
{
    ...
    {
        rc = -EFAULT;
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): ERROR: \r\n", __FUNCTION__);
    }
    return rc;
}
...
static HAL_CAM_Result_en_t CAM_PowerUp(CamSensorSelect_t sensor)
{
	...
	CamIntfConfig_st_t *psensor_intf_config;
         int Count;
	CAM_DEBUG(DBG_INFO,( "Camera - %s called, sCamState = %d\n", __FUNCTION__, sCamState ));
#ifdef CONFIG_BCM_CAM_OV2655
    //DVDD,DOVDD and AVDD have to be turned on/off together.Or leak current will happen.  
#else
    if (cam_regulator && !IS_ERR(cam_regulator) && !regulator_is_enabled(cam_regulator))
...
// Configure Camera Controller for Initialization
    if ( Cam_CfgCmiController( SensorPwrUp, psensor_intf_config ) != HAL_CAM_SUCCESS )
    {
        result |= HAL_CAM_ERROR_INTERNAL_ERROR;
    }
// Init Power-up Sequence for Sensor (if needed)
    if ( Cam_SensorIntfSeqSel(sensor, SensorPwrUp) != HAL_CAM_SUCCESS )
    {
        result |= HAL_CAM_ERROR_INTERNAL_ERROR;
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): Cam_SensorIntfSeqSel(): ERROR \r\n", __FUNCTION__);
    }
...
// Test for proper state
    if ( (sCamState != CAM_STATE_OFF) && (sCamState != CAM_STATE_ON))
    {
        ...
        {
            result |= HAL_CAM_ERROR_INTERNAL_ERROR;
            HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): ERROR:  Camera not in ON state \r\n", __FUNCTION__);
        }
    //this function puts camera in sleep mode.
...
// Wake Up Camera Device
    ...
    {
        result |= HAL_CAM_ERROR_INTERNAL_ERROR;
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): CAMDRV_Wakeup(): ERROR: \r\n", __FUNCTION__);
    }
    sCamState = CAM_STATE_IDLE;
    sCamState_Save = sCamState;
    HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_FUNCTION, "%s(): End \r\n", __FUNCTION__);
    return result;
}
...
static HAL_CAM_Result_en_t CAM_Enable(CamSensorSelect_t sensor)
{
	...
	/* Initialize control structure */
        ...
        {
            result |= HAL_CAM_ERROR_INTERNAL_ERROR;
            HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): camSetUpCapture(): ERROR \r\n", __FUNCTION__);
        }

        if (sCaptureMode == CamCaptureStillnThumb) {
            if ( CAMDRV_CfgStillnThumbCapture(sCaptureSize, sCaptureFormat, stThumbnailParm.size_window.size, stThumbnailParm.format, sCurrentSensor) != HAL_CAM_SUCCESS ) {
                result |= HAL_CAM_ERROR_INTERNAL_ERROR;
                HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): CAMDRV_CfgStillnThumbCapture():  ERROR: \r\n", __FUNCTION__);
            }
            ...
            if ( CAMDRV_SetVideoCaptureMode(sSensorOutputSize, sCaptureFormat, sCurrentSensor) != HAL_CAM_SUCCESS )
            {
                result |= HAL_CAM_ERROR_INTERNAL_ERROR;
                HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): CAMDRV_SetVideoCaptureMode():  ERROR: \r\n", __FUNCTION__);
            }
            else
            {
                // Set Frame Rate
                ...
                {
                    result |= HAL_CAM_ERROR_INTERNAL_ERROR;
                    HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): CAMDRV_SetFrameRate(): ERROR: \r\n", __FUNCTION__);
                }
                // Enable Camera Video streaming Mode
                if ( CAMDRV_EnableVideoCapture(sCurrentSensor) != HAL_CAM_SUCCESS )
                {
                    result |= HAL_CAM_ERROR_INTERNAL_ERROR;
                    HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): CAMDRV_EnableVideoCapture(): ERROR: \r\n", __FUNCTION__);
                }
                else
                {
                    HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_FUNCTION, "%s(): CAMDRV_EnableVideoCapture():  OK \r\n", __FUNCTION__);

                    // Set Camera Sensor Zoom if Capable
                    if ( Cam_ConfigCamSensorZoom(TRUE, sCurrentSensor) != HAL_CAM_SUCCESS )
                    {
                        result |= HAL_CAM_ERROR_INTERNAL_ERROR;
                        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): Cam_ConfigCamSensorZoom(): ERROR: \r\n", __FUNCTION__);
                    }
    ...
    // Set State for Capture
        sCamState_Save = sCamState;
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_PARAMETERS, "%s(): image size=0x%x image format=0x%x sCamOutputWidth=%d sCamOutputHeight=%d sCamWidth=%d sCamHeigth=%d \r\n", __FUNCTION__, sCaptureSize,sCaptureFormat,sCamOutputWidth,sCamOutputHeight,sCamWidth,sCamHeight);
    ...
    {
        result |= HAL_CAM_ERROR_INTERNAL_ERROR;
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): ERROR: NOT IN IDLE STATE state=%d \r\n", __FUNCTION__, sCamState);
    }
    return result;
}
...
static HAL_CAM_Result_en_t CAM_Disable(CamSensorSelect_t sensor)
{
    ...
    // Disable DMA
        ...
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_INFO, "%s():  Stop DMA: \r\n", __FUNCTION__);
        if ( dma_free_irq( gCamConfig.dmaChannel ) != 0 )
        {
            CAM_DEBUG(DBG_ERROR,("Camera - Failed to get DMA free IRQ \n"));
            result |= HAL_CAM_ERROR_INTERNAL_ERROR;
        }
        ...
        dma_free_chan( gCamConfig.dmaChannel );
				...
				for (i = 0; i < DMA_LLI_BUFFERS; i++)
				{
					...
					{
						dma_free_coherent( NULL, gCamLinkedList[i].sizeInBytes,
    ...
    // Halt Sensor Output
        if ( (sCaptureMode == CamCaptureStill) || (sCaptureMode == CamCaptureStillnThumb) )
        {
            ...
            {
                result |= HAL_CAM_ERROR_INTERNAL_ERROR;
                HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s():  CAMDRV_DisableCapture(): ERROR \r\n", __FUNCTION__);
            }
        }
        else
        {
            ...
            {
                result |= HAL_CAM_ERROR_INTERNAL_ERROR;
                HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s():  CAMDRV_DisablePreview(): ERROR \r\n", __FUNCTION__);
            }
        }
    // State to Idle mode
        sCamState = CAM_STATE_IDLE;
    ...
    {
        result |= HAL_CAM_ERROR_INTERNAL_ERROR;
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "%s(): ERROR: NOT IN CORRECT STATE state=%d \r\n", __FUNCTION__, sCamState);
    }
    return result;
}
...
// Verify valid config pointer
    ...
    // Set the Vsync/Hsync for external,embedded
        if (pCmiIntfConfig->sensor_config_ccir656->ext_sync_enable == TRUE)
        {
            CAM_CNTRL_SetSYNC(TRUE);
        }
  // Modify Vsync level if not default IRQ_ACTIVE_LOW
        if (pCmiIntfConfig->sensor_config_ccir656->vsync_polarity == SyncActiveHigh)
        {
  ...
  // Set the Hsync level in which to recieve sensor data
        if (pCmiIntfConfig->sensor_config_ccir656->hsync_polarity == SyncActiveLow)
        {
            CAM_CNTRL_SetHsyncInv(TRUE);
        }
  // Set the Hsync Control if not defautl FALSE
        if (pCmiIntfConfig->sensor_config_ccir656->hsync_control == TRUE)
        {
            CAM_CNTRL_SetHsyncControl(TRUE);
        }
  // Set the Clock edge to sample data on if not default IRQ_RISING_EDGE
        if (pCmiIntfConfig->sensor_config_ccir656->data_clock_sample == SyncFallingEdge)
        {
            CAM_CNTRL_SetClockInv(TRUE);
        }

  // Set Color Conversion registers for RGB565 conversion (if needed)
        CAM_CNTRL_SetColorConversion( pCmiIntfConfig->sensor_config_ycbcr->cc_red, pCmiIntfConfig->sensor_config_ycbcr->cc_green, pCmiIntfConfig->sensor_config_ycbcr->cc_blue );
  ...
  // Configure YCbCr Input Format
        ...
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_INFO, "%s(): sensor_yc_seq=%d \r\n", __FUNCTION__, pCmiIntfConfig->sensor_config_ycbcr->sensor_yc_seq);
            

    // Set Controller Window to maximum image size
        CAM_CNTRL_SetWindow(0,pCmiIntfConfig->sensor_config_caps->output_st.max_height,0,pCmiIntfConfig->sensor_config_caps->output_st.max_width);
    }
    return result;
}
...
static HAL_CAM_Result_en_t Cam_SensorIntfSeqSel(CamSensorSelect_t nSensor, CamSensorSeqSel_t nSeqSel)
{
    ...
    {
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_INFO, "%s(): No Sequence\r\n", __FUNCTION__);
    }
    return result;
}
...
static HAL_CAM_Result_en_t Cam_SensorCntrlSeq(CamSensorIntfCntrl_st_t *seq, UInt32 length)
{
    ...
    HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_FUNCTION, "%s(): \r\n", __FUNCTION__);
    HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_FUNCTION, "Sequence Length=%d: \r\n", length/sizeof(CamSensorIntfCntrl_st_t));
    ...
    for(i = 0; i < length/sizeof(CamSensorIntfCntrl_st_t); i++)
    {
		...
		//gpio_direction_output( seq[i].value, 0 );
                ...
                {                
                  HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_FUNCTION, "GPIO_CNTRL(%d), GPIO_SetHigh\n", seq[i].value);
			if (gpio_is_valid(seq[i].value))
				gpio_direction_output (seq[i].value, 1);
                }
                else
                {
                  HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_FUNCTION, "GPIO_CNTRL(%d), GPIO_SetLow\n", seq[i].value);                
			if (gpio_is_valid(seq[i].value))
				gpio_direction_output (seq[i].value, 0);
                }
            ...
            case MCLK_CNTRL:
                ...
                {
                  HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_FUNCTION, "MCLK_CNTRL - Freq=%d\n", seq[i].value);                                
                  CAM_CNTRL_SetClkFreq((CamClkSel_t)seq[i].value);    //Enable camera clock output from BCM baseband chip
                }
                else
                {
                  HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_FUNCTION, "MCLK_CNTRL - NoClk\n", seq[i].value);                                
            ...
            case PAUSE:
                if (seq[i].value != 0)
                {
                  HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_FUNCTION, "PAUSE - %d ms\n", seq[i].value);                                
                  mdelay(seq[i].value);
                }
                break;

            default:
                HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_FUNCTION, "CNTRL - Not Supported\n");                                
                result = HAL_CAM_ERROR_ACTION_NOT_SUPPORTED;
                break;
        }
    }
...
static HAL_CAM_Result_en_t Cam_CfgSensorI2c( CamIntfConfig_st_t *pI2cIntfConfig )
{
    ...
    {
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_ERROR, "Cam_SetupModule(): Cam_CfgSensorI2c(): ERROR:  pI2cIntfConfig == NULL\r\n");
        return HAL_CAM_ERROR_INTERNAL_ERROR;
    }
...
HAL_CAM_Result_en_t CAM_SetFlashEnable(
    Boolean enable                    ///< (in) True for enable
    )
{
    HAL_CAM_Result_en_t result = HAL_CAM_SUCCESS;
...
// -----------Flash Enable/Disable Sequence for Sensor (if needed)
    if(enable)
    {
        result |= Cam_SensorIntfSeqSel(sCurrentSensor, SensorFlashEnable);
    }
    else
    {
        result |= Cam_SensorIntfSeqSel(sCurrentSensor, SensorFlashDisable);
    }
    return result;
}
...
// Set the Vsync edge to trigger on
    if ( (pSyncIntfConfig->sensor_config_ccir656->vsync_irq_trigger == SyncRisingEdge) ||
        (pSyncIntfConfig->sensor_config_ccir656->vsync_irq_trigger == SyncActiveHigh) )
    {
...
// Clear any pending interrupts and register vsync interrupt handler.
    ...
    if (rc != 0)
    {
        CAM_DEBUG(DBG_ERROR,("Camera - Failed to register vsync ISR\n"));
        result |= HAL_CAM_ERROR_INTERNAL_ERROR;
    }
    disable_irq( IRQ_VSYNC );
...
// Set width and height of output image (No Zoom on Stills Image Capture)
    ...
    stNewCamWindowConfig.window_X = (cntl_out_width * stNewCamWindowConfig.sdfH) >> 7;
    stNewCamWindowConfig.window_Y = (cntl_out_height * stNewCamWindowConfig.sdfV) >> 7;
...
// Set Controller Window, Scaling, and DMA
    ...
    HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_PARAMETERS, "%s(): Controller Width=%d Height=%d format=0x%x resolution=0x%x \r\n", __FUNCTION__, stCamWindowConfig.window_X,stCamWindowConfig.window_Y,sCaptureFormat,resolution);
    return result;
}
...
// Get actual width and height of output image
    if ( (sCaptureFormat != CamDataFmtJPEG) && (sCaptureFormat != CamDataFmtYUV) )
    {
  ...
  // Set Padding value if needed (Requested size > than scaled size)
    ...
    {
        HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_WARNING, "%s(): ERROR:  pad_X != 0 \r\n", __FUNCTION__);
        result |= HAL_CAM_ERROR_INTERNAL_ERROR;
    }
// Set CMI Controller Windowing & Scaling
    ...
    sCamReturnWidth = pcam_window_config->dma_X + (pcam_window_config->pad_X*2);
    sCamReturnHeight = pcam_window_config->dma_Y + pcam_window_config->pad_Y;
    sNewCamWindowStatus = FALSE;

// Update new Settings for CMI Windowing & Scaling in stCamWindowConfig
    memcpy( &stCamWindowConfig, pcam_window_config, sizeof stCamWindowConfig );
    HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_PARAMETERS, "%s(): output_res_width=%d  output_res_height=%d  dma_x=%d  dma_y=%d  sensor resolution=0x%x  zoom=%d\r\n",__FUNCTION__,sCamOutputWidth,sCamOutputHeight,sCamWidth,sCamHeight,stCamWindowConfig.image_size,sZoomFactor);
    HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_PARAMETERS, "%s(): controller window width=%d  controller window height=%d  sdf_X%d  sdf_Y=%d\r\n",__FUNCTION__,(end_x-start_x),(end_y-start_y),stCamWindowConfig.sdfH,stCamWindowConfig.sdfV);
    HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_PARAMETERS, "%s(): start_x=%d  end_x=%d  pad_x=%d  start_y=%d  end_y=%d  pad_y=%d\r\n",__FUNCTION__,start_x,end_x,stCamWindowConfig.pad_X,start_y,end_y,stCamWindowConfig.pad_Y);
...
// --------Check for Camera Sensor Zooming Capability
    if ( ( pSensorIntfConfig->sensor_config_caps->digital_zoom_st.capable == TRUE ) && (sZoomFactor != CamZoom_PP) )
    {
        result = CAMDRV_SetZoom(sZoomFactor, nSensor);
        if (result |= HAL_CAM_SUCCESS)
        {
            HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_WARNING, "%s(): ERROR:  sZoomFactor=%d\r\n", __FUNCTION__, sZoomFactor);
        }
        else
        {
            HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_PARAMETERS, "%s(): sZoomFactor=%d\r\n", __FUNCTION__, sZoomFactor);
        }
    }
    return result;
}
...
// allocate DMA buffers
    for ( buffer = 0; buffer < buffer_cnt ; buffer++ )
    {
        gDmaBuffer[buffer].sizeInBytes = pcam_window_config->dma_X * pcam_window_config->dma_Y;
        gDmaBuffer[buffer].physPtr = gCamBootBuffer.physPtr + cam_bootmem_offset;
    ...
    if ( result != HAL_CAM_SUCCESS )
    {
        CAM_DEBUG(DBG_ERROR,("Camera - Failed to allocate DMA buffer\n"));
    }

// set up the DMA linked list
    result |= camDmaCreateList( pcam_window_config->dma_X, pcam_window_config->dma_Y, mode );
    if ( result != HAL_CAM_SUCCESS )
    {
        CAM_DEBUG(DBG_ERROR,("Camera - Failed to create DMA linked list\n"));
    }
...
// request DMA channel and DMA irq
    ...
    if ( dma_request_chan( gCamConfig.dmaChannel, "camera" ) != 0 )
    {
        ...
        CAM_DEBUG(DBG_ERROR,("Camera - Failed to get desired DMA channel\n"));
        if ( dma_request_avail_chan( &gCamConfig.dmaChannel, "camera" ) != 0 )
        {
            CAM_DEBUG(DBG_ERROR,("Camera - Failed to get DMA channel\n"));
            result |= HAL_CAM_ERROR_INTERNAL_ERROR;
        }
    }
    ...
    {
        CAM_DEBUG(DBG_ERROR,("Camera - Failed to get dma irq\n"));
        result |= HAL_CAM_ERROR_INTERNAL_ERROR;
    } else {
    	dma_disable_irq( gCamConfig.dmaChannel );
    }
    HAL_CAM_Trace(HAL_CAM_TRACE_HALDRV|HAL_CAM_TRACE_PARAMETERS, "%s(): channel=%d  mode=%d  size=0x%x dma_X=%d  dma_Y=%d  ptr0=0x%x ptr1=0x%x\r\n", __FUNCTION__, gCamConfig.dmaChannel,mode,gDmaBuffer[0].sizeInBytes,pcam_window_config->dma_X,pcam_window_config->dma_Y,gDmaBuffer[0].physPtr,gDmaBuffer[1].physPtr);
    return result;
}
#if defined(CONFIG_BCM_CAM_STV0986)
//!
...
static int Cam_MarkerSearch(void *buf)
{
	...
	// check EOI ( 0xFF 0xD9 )
	if ((*(ptr + sCaptureJpegSize - 2) != 0xFF) || (*(ptr + sCaptureJpegSize - 1) != 0xD9))
		return -1;
...
HAL_CAM_Result_en_t CAM_WriteI2c( UInt16 camRegID, UInt16 DataCnt, UInt8 *Data )
{
	...
	if (!DataCnt || (DataCnt > MAX_I2C_DATA_COUNT)) {
		result = HAL_CAM_ERROR_INTERNAL_ERROR;
		CAM_DEBUG(DBG_ERROR,("%s() - DataCnt = %d out of range\n", __FUNCTION__, DataCnt));
		goto done;
	}
	...
	if ((cam_i2c_datap) && (adap = cam_i2c_datap->client->adapter)) {
		...
		struct i2c_msg msg = { cam_i2c_datap->client->addr,  cam_i2c_datap->client->flags, sizeof(camRegID)+DataCnt, msgbuf };
		...
		CAM_DEBUG(DBG_ERROR,("%s() - Camera I2C adapter null\n", __FUNCTION__));
		result = HAL_CAM_ERROR_INTERNAL_ERROR;
	}
done:
	return result;
...
HAL_CAM_Result_en_t CAM_ReadI2c(UInt16 camRegID, UInt16 DataCnt, UInt8 *Data)
{
	...
	if ((cam_i2c_datap) && (adap = cam_i2c_datap->client->adapter)) {
		...
		struct i2c_msg msg[2] = {
			{ cam_i2c_datap->client->addr,  cam_i2c_datap->client->flags, sizeof(camRegID), msgbuf0 },
			{ cam_i2c_datap->client->addr,  cam_i2c_datap->client->flags | I2C_M_RD, DataCnt, Data }
		};
